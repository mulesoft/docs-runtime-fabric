...
<<<<<<< HEAD
Use the same operating system for each Runtime Fabric node. Running different operating systems on different nodes is not supported. If you attempt to install Runtime Fabric on VMs / Bare Metal on a different operating system versions or distributions, the installer fails.
=======
