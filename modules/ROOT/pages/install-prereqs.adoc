...
On Runtime Fabric instances running version 1.4.x and later, you can use the Runtime Manager *Settings* page to enable mutual 
authentication for the cluster. Enabling mutual authentication provides better security for communications between the Runtime Fabric cluster and Anypoint Platform and is highly recommended.

Before you enable mutual authentication, verify that the following endpoint and port is whitelisted and reachable from the cluster:
