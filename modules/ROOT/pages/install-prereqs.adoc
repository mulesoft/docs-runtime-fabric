...
* Creating separate runtime rabrics for production and non-production applications
* Creating separate runtime rabrics based upon environment network scope, such as DMZ versus private network 
...
.2+| 5044 (deprecated) .2+| TCP (Lumberjack) | US control plane: `dias-ingestor-nginx.prod.cloudhub.io` | Anypoint Monitoring agent for Runtime Fabric. The use of this port is deprecated in Runtime Fabric version 1.8.48. If you are using a previous version you must add port 5044 to your allow list. If you are using version 1.8.48 or higher you must add port 443 to your allow list.
| EU control plane: `dias-ingestor-nginx.prod-eu.msap.io` | Anypoint Monitoring agent for Runtime Fabric. Note, this
...
| EU control plane: `dias-ingestor-router.us-east-1.prod.cloudhub.io` | Anypoint Monitoring agent for Runtime Fabric.
