...
:page-aliases: install-sys-reqs.adoc
...
[NOTE]
--
CentOS 8 has reached https://www.centos.org/centos-linux-eol/[end-of-life^]. Consider using an alternative operating system.
--
...
[IMPORTANT]
====
There is a known control groups (cgroups) memory leak issue which can lead to pod creation failure or node malfunction in RHEL and CentOS 7. See the  https://help.salesforce.com/s/articleView?id=001122329&type=1[Knowledge Base Article] for additional information.
...
* EU control plane: `runtime-fabric-eu.s3.eu-central-1.amazonaws.com` | Runtime Fabric Docker repository.
| 443 | HTTPS a| 
...
[NOTE]
If you're using VMWare and are unable to connect to your existing applications or deploy new applications refer to https://help.salesforce.com/s/articleView?id=001120377&type=1[RTF Networking Issues with VMware^] in the MuleSoft Knowledge Base. 
...
controllers are not supported. Keep in mind that single-controller clusters won't be recoverable in the
event of corruption of the etcd database. For example the issue described in https://help.salesforce.com/s/articleView?id=001121251&type=1[this knowledge article^].
