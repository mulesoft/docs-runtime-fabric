...
* CentOS v7.4, v7.5, v7.6, v7.7, v7.8
...
=== Mutual Authentication with Anypoint

For Runtime Fabrics running version 1.4.x and above, the user have the option to enable mutual authentication with 
Anypoint connectivity for the cluster from the  settings page in Runtime Manager. Enabling mutual authentication will
provide better security for communications between the Runtime Fabric cluster and Anypoint platform, and is highly recommended.

Before you enabled multual authentication, please make sure the following endpoint and port is whitelisted and reachable from the cluster:
