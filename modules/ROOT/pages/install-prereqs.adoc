...
[IMPORTANT]
====
There is a known control groups (cgroups) memory leak issue which can lead to pod creation failure or node malfunction in RHEL 7.x. See  https://help.mulesoft.com/s/article/RTF-How-to-Resolve-the-Cgroup-Memory-Leaking-Issue-in-Runtime-Fabric[RTF - How to Resolve the Cgroup Memory Leaking Issue in Runtime Fabric] for additional information.
