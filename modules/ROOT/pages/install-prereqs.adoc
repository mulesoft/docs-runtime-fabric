...
.2+| 443 .2+| AMQP over WebSockets | US control plane: `transport-layer.prod.cloudhub.io` | Runtime Fabric queue messaging broker to interact with the control plane.
| EU control plane: `transport-layer.prod-eu.msap.io` | 
.2+| 5044 .2+| TCP (Lumberjack) | US control plane: `dias-ingestor-nginx.prod.cloudhub.io` | Runtime Fabric Anypoint Monitoring.
