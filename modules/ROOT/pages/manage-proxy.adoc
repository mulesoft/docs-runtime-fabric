...
Usually after applying a proxy for RTF services you will see a warning like this:
----
Warning: Proxy configurations in the environment "HTTP_PROXY: " does not match configurations in Runtime Fabric "YOUR_PROXY"
----

This warning is exactly that. A warning to let you know that the environment variable HTTP_PROXY is either unset or does not match the value being used for Runtime Fabric services. It does not mean that these settings should match, is a warning for you to be aware of the impact this may have.

Many Linux tools leverage the HTTP_PROXY environment variable to provide for its proxy setting. Keep this in mind when deciding which value is appropriate. If your environment requires a proxy for Internet access you will likely set this environment variable for commands that require that access to succeed. 

Keep in mind you may be affecting other tools that also use HTTP or HTTPS operations but without Internet access. For example the kubectl tool. Usually this behavior is controlled by using a NO_PROXY environment variable that configures the connections that should not be proxied.

Refer to each propietary tool documentation for more information on which environment variables affect it.
...
* https://help.mulesoft.com/s/article/Warning-Proxy-configurations-in-the-environment-HTTP-PROXY-does-not-match-configurations-in-Runtime-Fabric[Warning: Proxy configurations in the environment "HTTP_PROXY: " does not match configurations in Runtime Fabric]
