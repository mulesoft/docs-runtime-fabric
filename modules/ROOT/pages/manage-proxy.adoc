...
The proxies used by Runtime Fabric can be configured during or after installation using the rtfctl utility. The proxies for applications are configured during application deployment.
...
You can set up a proxy on each connector (such as an HTTP requestor) or set up a proxy with application properties as shown in the following example. Setting up a proxy with application properties is less work if you have many connectors in an application.
----
