...
You can update the proxy settings used by Anypoint Runtime Fabric outbound connections to the internet. This includes managing both HTTP proxies for connecting to the Anypoint Platform control plane and SOCKS5 proxies for sending metrics and logs to Anypoint Monitoring.
...
+
----
rtfctl apply http-proxy "http://<user>:<pass>@<10.0.0.1>:<8080>" --no-proxy "<1.1.1.1:8888,2.2.2.2:9999>" --confirm
----
+
...
. Update the `/opt/anypoint/runtimefabric/env` file to set values for both the `RTF_HTTP_PROXY` and `RTF_NO_PROXY` variables. + 
