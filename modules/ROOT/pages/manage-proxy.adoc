...
. Obtain the https://runtime-fabric.s3.amazonaws.com/rtfctl/rtfctl-0.2.30[latest version] of `rtfctl` and transfer to a controller node under the `/usr/local/bin` folder with the name `rtfctl`. You may need root access to perform this operation.
...
.. *<pass>*: the password needed to authenticate to the SOCKS5 proxy. Note: the password should be URL encoded.
...
.. *<pass>*: the password needed to authenticate to the HTTP proxy. Note: the password should be URL encoded.
