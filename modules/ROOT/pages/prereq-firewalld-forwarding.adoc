...
To determine if your system is using firewalld, run this command:
...
If firewalld is installed, this command returns this:
...
To enable forwarding on the docker0 device, run these commands:
...
To enable forwarding on the 10.0.0.0/8 subnet, run these commands:
...
To verify that forwarding rules are in place, run this command:
