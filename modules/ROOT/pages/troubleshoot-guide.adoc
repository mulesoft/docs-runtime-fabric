...
== Troubleshoot expired client certificate
 
If your cluster already uses xref:config-mutual-auth.adoc[Mutual Authentication with Anypoint Platform] the client certificate will be periodcally renewed by a cron job called `certificate-renewal` on the `rtf` namespace. This job runs in advance of certificate expiration as it needs to be renewed before expiring. Unexpected circumstances may cause the certificate to eventually expire without renewal and this job will log the following error:
----
...
To fix this situation you will need to use the `rtfctl appliance renew-expired-client-cert` command.
For more information refer to:

* xref:commands-for-managing-the-runtime-fabric-appliance.adoc[Commands for Managing the Runtime Fabric Appliance]
