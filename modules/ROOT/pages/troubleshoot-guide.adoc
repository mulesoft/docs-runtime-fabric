...
== Troubleshoot Nodes in Promiscuous Mode

For services that load-balance to themselves, you need to configure hairpin NAT using a dedicated configuration parameter for kubelet in the Image Manifest, for example:
...
The `hairpinMode` attribute accepts the same values as kubelet. Refer to the https://kubernetes.io/docs/reference/command-line-tools-reference/kubelet/[kubelet command line reference^] for details.

With `hairpin-veth`, the kubelet/network plugin handles the setup.

With `promiscuous-bridge`, the behavior is similar to that of the kubenet network plugin:

* The Docker bridge is placed into promiscuous mode
* ebtables rules are set up to avoid duplicate packets as the bridge accepts traffic otherwise destined for other interfaces.

In `promiscuous-bridge` mode, nodes require a kernel module called `ebtable_filter` to manage ebtable rules.
...
$ echo 'ebtable_filter' > /etc/modules-load.d/network.conf
----
