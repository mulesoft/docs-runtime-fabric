...
The reason is that the metrics-server is not running. This deployment could reside in either the kube-system o the monitoring namespace, depending on your appliance version. The events and logs for this deployment and associated pods have to be investigated for further troubleshooting. Once the metrics-server is up and running the rtfd will come up as expected.
