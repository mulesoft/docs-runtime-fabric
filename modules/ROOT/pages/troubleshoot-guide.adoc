...
=== CrashLoopBackOff - Couldn't initialize inotify

You are noticing pods in ``CrashLoopBackOff`` state and describing thos pods you oberve and error event like the following:
----
[Kubernetes] Container "anypoint-monitoring" - CrashLoopBackOff - Couldn't initialize inotify
----

This is caused by the user running RTF maxing out INotify instances, because of a low value in the ``inotify.max_user_instances`` kernel setting. This value should be set to ``1048576``. The installer automatically performs this configuration, so the most likely cause is that another process or user changed this setting to a lower value.

You may fix the situation running the following command:
----
...
You are advised to review this situation with your system administrator to make sure that this value is not changed again in the future.
