...
If you experience connectivity issues in the monitoring sidecar container of application deployment pods, review the xref:install-self-managed-network-configuration.adoc#port-configuration[networking port prerequisites] to unblock all required ports. If you decide not to allow Anypoint Monitoring, Anypoint Visualizer port 5044 may report errors even though functionality is not affected. For more details, see: https://help.mulesoft.com/s/article/Failed-to-connect-to-failover-ERROR-logs-in-RTF-application-pod-sidecar-container["Failed to connect to failover" ERROR logs in RTF application pod sidecar container^].
...
If you host your Runtime Fabric appliance on a VMware environment and are unable to deploy any applications, refer to the following MuleSoft knowledge base article: https://help.mulesoft.com/s/article/RTF-Networking-Issues-with-VMware[RTF Networking Issues with VMware and "[Runtime Fabric\] java.net.UnknownHostException:" Errors^].
...
If one or more of the RTF Agent pod containers are not starting up check their logs and look for an error message matching an entry here for troubleshooting instructions to known issues.

=== Metrics server not running

The rtfd container is crashing in every attempt. Error message looks like the following:
----
...
The reason is that Kubernetes metrics-server is installed but not running. 
RTF appliance version 1.1.1636064094-8b70d2d and later includes metrics-server as a deployment in the monitoring namespace.
The events and logs for this deployment and associated pods have to be investigated for further troubleshooting. Once the metrics-server is up and running the rtfd will come up as expected.

Please reach out to our support team with a full appliance report if you require assistance to restore the metrics-server.
...
This is a limitation in Runtime Fabric on VMs / Bare Metal, as documented in xref:upgrade-index.adoc#before-you-upgrade-the-runtime-fabric-appliance[before you upgrade the Runtime Fabric appliance]. The suggested workaround is to back up any customizations before you run the upgrade and then reapply the customizations when the upgrade completes.
