...
In rare situations, the Anypoint Monitoring agent may cause an application to not deploy.  In these situations, you may see the following:
...
The Anypoint Monitoring agent may also change the state of a deployed application. If you see one of the following:

* The application transitions from `Running` to `Applying`, or
* `Monitoring agent has exited with code -1`

This indicates that the agent is restarting. There should be no impact to the running application. Application metrics will be queued and resume being collected after the agent restarts.
...
CRITICAL / Kubernetes node is not ready: 168.189.5.22

CRITICAL / etcd: cluster is unhealthy
----
Run the `gravity status` command to obtain the health status of the cluster as well as individual components. For example, to remotely log in to a cluster and check the status of the cluster named `production`, run the following command:
----
$ tsh --cluster=production ssh admin@node gravity status
----

To recover a node, follow the instructions provided in https://gravitational.com/gravity/docs/cluster/#recovering-a-node[Recovering a Node, role=external, window=_blank].
...
If manual installation of a package was successful, or if you still require assistance, please file a support ticket.
