...
Scenario: After an agent upgrade, you encounter the following error status in Runtime Manager: `Degraded : Runtime Fabric agent version mismatch detected from control plane`. This error is caused by a discrepancy between the agent version stored on Anypoint Platform for that cluster and the version reported by the agent status probe.
...
In this situation, an agent upgrade fails, and you roll it back, but the desired (upgraded) version is the version listed in Anypoint Platform. To correct this issue, apply the procedure described in https://help.mulesoft.com/s/article/Degrade-Status-as-Runtime-Fabric-Agent-Version-Mismatch-Detected[Degrade Status as Runtime Fabric Agent Version Mismatch Detected^].

=== Resotration of Pre-Upgrade Backups

If you restore a backup that was taken on a previous agent version than the latest version applied from the control plane for that cluster, you must repeat the upgrade procedure. For more information, refer to xref:manage-backup-restore.adoc#changes-made-after-performing-a-backup-are-not-restored[Changes Made After Performing a Backup Are Not Restored].
