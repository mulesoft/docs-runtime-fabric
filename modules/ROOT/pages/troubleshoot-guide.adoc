...
If your cluster uses xref:config-mutual-auth.adoc[Mutual Authentication with Anypoint Platform], a cron job called `certificate-renewal` in the `rtf` namespace periodically renews the client certificate. This job runs in advance of certificate expiration as it needs to be renewed before expiring. Unexpected circumstances may cause the certificate to eventually expire without renewal. In such cases, the `certificate-renewal` job logs the following error:
----
