...
If you have outbound connectivity issues that prevent Runtime Fabric from reaching any of the required Anypoint control plane services, work with your network team to verify that you have completed the required allowlisting as described in xref:install-prereqs#port-ip-addresses-and-hostnames-to-allowlist[Port IPs and Hostnames to Allowlist].
