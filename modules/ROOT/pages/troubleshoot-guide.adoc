...
Current image libraries do not include support for this protocol version.
If your log aggregator is not acepting TLVv1.2 you will experience that your logs are not being forwarded and warnings like these on your external log forwarder pods:
...
On your log aggregator you may have an error message like the following as well:
...
You will have to either enable TLSv1.2 or set your networking infrastrcuture to have an intermediate node terminate the TLSv1.2 session and create a new TLSv1.3 to the final destination. External log forwarder libraries do not support TLSv1.3 at the time of this writing.
