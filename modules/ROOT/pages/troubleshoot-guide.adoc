...
After an appliance upgrade you are realizing that the customizations to your email alerts are lost and alert emails back to their default values.

This is a known limitation of the appliance and documented as part of the upgrade process xref:upgrade-index.adoc#before-you-upgrade-the-runtime-fabric-appliance[Before you upgrade the Runtime Fabric appliance]. The current alternative is to backup those customizations before the upgrade and then carefully reapply afterwards.
