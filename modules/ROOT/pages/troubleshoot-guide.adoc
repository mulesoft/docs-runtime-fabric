...
== Verify Chrony is Synchronized 

After you install chrony:

. Verify that chrony is enabled by running the following command:
+
`systemctl enable chronyd`
. Verify that the  Network Time Protocol (NTP) is disabled and is not running:
+
```
systemctl stop ntpd
...
. Verify the `Leap status` of chronyc tracking:
+
```
chronyc tracking
```
+
Ensure that the `Leap status` of chronyc tracking indicates `Normal`, as shown in the following example output:
+
```
...
. Verify that chrony sources are synchronized:
+
