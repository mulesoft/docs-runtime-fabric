...
Run the `rtfctl report` command to generate an archive containing only Kubernetes objects and logs.

Run the `rtfctl appliance report` command to collect diagnostics from all cluster nodes.
