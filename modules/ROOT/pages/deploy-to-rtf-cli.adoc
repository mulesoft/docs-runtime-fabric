...
The following example shows how to deploy a Mule appl to Runtime Fabric using Anypoint CLI.

=== Configure your Anypoint CLI Space

To configure your CLI space, download and install `anypoint-cli-v4`:
...
+
[start=2]
. Install the `runtime-mgr` plugin by running this command:
+
[source,copy]
----
...
To set other parameters for your space, see xref:anypoint-cli::auth.adoc[Authentication to the Anypoint Platform CLI].
After your CLI space is set, deploy and manage Mule applications to Runtime Fabric.
...
Using `test-mule-1` version as default: v1.0.6 +
After deploying `new-app01`, the output log is:
...
To modify an existing Mule app, you need the app ID. Obtain the app ID from the list of applications. If you want to modify the app that you just deployed, the app ID is in the response of that deployment.
