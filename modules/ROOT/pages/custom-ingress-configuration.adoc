...
<6> The `path` field is updated with the value specified in the *Path* field when configuring path-based routing for an xref:configure-ingress-for-a-mule-application[ingress] in your Mule application.(TODO:refer to the example).
    Note that `path` is not a placeholder. 
...
<Include this/similar screenshot.. I could not attach so will drop in the PR description at the end>
The resulting application ingress from this template will have the `path` field replaced with the ones configured on the Application deployment.
In this example application `myapp`, the routing has been configured with the paths `/foo` and `/bar`.
...
Note:
Refer to the documentation for your ingress controller and configure the path-based routing ingress template accordingly.
