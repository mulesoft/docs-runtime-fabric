...
+
As of v1.22, the `extensions/v1beta1` and `networking.k8s.io/v1beta1` API versions of Ingress are no longer available.
* Kubernetes version v1.18 and earlier: 
...
+
This is how Runtime Fabric recognizes the object as a template. A template that uses the `rtf-` prefix in the `spec.ingressClassName` field or in the `kubernetes.io/ingress.class` annotation, for example, `rtf-nginx`, is consumed by the Runtime Fabric agent only and not by the actual ingress controller. Ingress controllers discover only resources with an `spec.ingressClassName` value that uses the vendor-specific name, for example, `nginx` or `haproxy`.

<3> TLS is optional.
+
If the TLS section is added with a `secretName` its the responsibility of the user to make sure that the referred TLS secret is created in the RTF namespace.

<4> `app-name` is a placeholder parameter that will be replaced with actual app name when you deploy the application. It helps to ensure that each endpoint name is unique if there is not a wildcard in the subdomain.
...
<1> A template can include multiple hosts, but Runtime Manager displays the first path rule only.

<2>  These are placeholder values required for Kubernetes validation, but the actual values are not used by Runtime Fabric.
...
[NOTE]
====
The following provides an overview of how to configure ingress for an application. Refer to xref:deploy-to-runtime-fabric.adoc[Deploy a Mule Application to Runtime Fabric] for complete deployment instructions.  If using the mule maven plugin to deploy applications to Runtime Fabric, the deployment settings parameter `publicUrl` in the block `http` can accept a comma-delimited string of multiple endpoints. 
...
|No domains configured and no ingress-ConfigMap applied |No template will be generated automatically after upgrade|Create a template as described above |-
