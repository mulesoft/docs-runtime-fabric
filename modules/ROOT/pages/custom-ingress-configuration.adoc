...
If you’re using AWS Load Balancer Controller for ingress, you must specify the `spec.ingressClassName: rtf-alb` in the template. AWS Load Balancer Controller requires the `spec.ingressClassNamegit` to discover and create L7 load balancers for deployed ingress resources for those annotations.
