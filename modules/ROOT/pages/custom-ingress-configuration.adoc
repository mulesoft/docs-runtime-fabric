= Configuring Ingress Resource Template in Runtime Fabric
...
[IMPORTANT]
====
The ingress controller included with Google Kubernetes Engine (GKE) provisions a separate HTTP load balancer per application by default. 
...
This diagram provides an overview of how to use ingress resource templates in Runtime Fabric:
...
These examples illustrate how to modify a Kubernetes ingress spec to create a Runtime Fabric ingress-resource template. 
...
Templates with `rtf-` prefix in the `spec.ingressClassName` field are consumed by the Runtime Fabric agent only and not by the actual ingress controller.
...
+
[NOTE]
`kubernetes.io/ingress.class` is deprecated by K8s > 1.18, for details refer to https://kubernetes.io/blog/2020/04/02/improvements-to-the-ingress-api-in-kubernetes-1.18/#deprecating-the-ingress-class-annotation[Improvements to the Ingress API in Kubernetes 1.18]
...
<6> The `path` parameter value is replaced by the value you add in the *Path* field when you xref:configure-ingress-endpoints.adoc#configure-ingress-for-a-mule-app[configure ingress for a Mule application]. Note that `path` is not a placeholder. 

<7> These placeholder values are required for Kubernetes validation, but the actual values are not used by Runtime Fabric. 

For a path-based routing ingress template, consult your ingress controller documentation and configure it accordingly. For an example using Nginx, see https://help.salesforce.com/s/articleView?id=001116839&type=1[Path-based Nginx Ingress Routing Example for Runtime Fabric on Self-Managed Kubernetes].

=== Template Placeholders 

These placeholders are optional. If you use a placeholder, replace it in the appropriate location in the template. Placeholders must be lowercase.
...
When upgrading to Runtime Fabric version 1.10, the upgrade process automatically translates any existing ingress ConfigMaps to the ingress resource template format, including annotations, path, namespace, and host parameters. Review this table to determine if any action is required after upgrading.

[%header%autowidth.spread]
|===
|Runtime Fabric Pre-upgrade |Runtime Fabric Upgraded |Action Required |TLS Support
|No domains configured and no ingress-ConfigMap applied |No template is generated automatically after the upgrade|Create a template. See xref:configure-openshift-routes.adoc#create-ingress-resource-template-and-apply-it-in-the-cluster[Create an Ingress Resource Template and Apply it in the Cluster].  |-
...
If you’re using AWS Load Balancer Controller for ingress, you must specify the `spec.ingressClassName: rtf-alb` in the template. AWS Load Balancer Controller requires the `spec.ingressClassName` to discover and create L7 load balancers for deployed ingress resources for those annotations.
