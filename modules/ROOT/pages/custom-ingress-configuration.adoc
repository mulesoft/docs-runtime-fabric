...
When using TLS in the Ingress template, you must also create the associated TLS secret. See the https://kubernetes.io/docs/concepts/services-networking/ingress/#tls[Kubernetes documentation] for details. 

When a TLS secret is updated, you can configure Runtime Fabric to synchronize it to all application namespaces by adding a `rtf.mulesoft.com/synchronized=true` label. This allows your ingress controller to immediately apply the updated TLS material.

For example, your TLS secret might look like 
```
apiVersion: v1
kind: Secret
metadata:
...
If you do not add this label, changes to the secret will not be synchronized into application namespaces. You must manually update the secret in each namespace to allow your ingress controller to apply the updated TLS material.
