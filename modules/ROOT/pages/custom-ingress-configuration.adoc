...
<6> The `path` parameter is updated with the value specified in the *Path* field when configuring path-based routing for an xref:configure-ingress-for-a-mule-application[ingress] in your Mule app. Note that `path` is not a placeholder. 
...
kind: Ingress
metadata:
  name: example-path-based-routing
  namespace: rtf                        
  labels:
spec:
...
The resulting Mule app ingress from this template has the `path` parameter replaced with the values you configured on the Mule app deployment.

In this example of the Mule app `myapp`, the routing is configured with the paths `/foo` and `/bar`:

[source,yaml]
--
# For the endpoint https://test.example.com/foo
apiVersion: networking.k8s.io/v1
kind: Ingress
…
…
...
[source,yaml]
--
# For the endpoint https://test.example.com/bar
apiVersion: networking.k8s.io/v1
kind: Ingress
…
…
...
image:rtf-ingress.png[Runtime Manager UI with ingress configuration window]

[NOTE]
Refer to the documentation of your ingress controller and configure the path-based routing ingress template accordingly.
