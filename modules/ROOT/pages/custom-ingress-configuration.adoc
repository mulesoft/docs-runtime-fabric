...
Other changes will be applied as soon as the `ServiceResourceWatcher`, a component of the RTF agent, processes the existing services again after picking up the ingress template change. Because of this it's required to restart the RTF Agent first thing after an existing ingress template change to force reconciliation of all services.
