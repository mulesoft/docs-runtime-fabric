...
For an example using Nginx, see https://help.salesforce.com/s/articleView?id=001116839&type=1[Path-based Nginx Ingress Routing Example for Runtime Fabric on Self-Managed Kubernetes].
...
Other changes take effect once the `ServiceResourceWatcher`, a component of the Runtime Fabric agent, processes the existing services after detecting changes in the ingress template. Therefore, restart the Runtime Fabric agent immediately after any changes to the ingress template to ensure all services are reconciled.
