...
Use the `unique-id` placeholder to create a unique application URL, for example, if you want to deploy two applications with same name from different business groups.

[NOTE]
--
* The `unique-id` placeholder is available in Runtime Fabric version 1.12.28 and later.
* If we use this `app-name-unique-id.com` in the path section the ingress template, the resultig ingress will look something like this `app-name-7jkbic.com`
--
