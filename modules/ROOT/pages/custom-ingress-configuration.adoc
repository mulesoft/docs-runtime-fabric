...
<4> The `app-name` placeholder parameter will be replaced with actual app name when you deploy the application. It helps to ensure that each endpoint name is unique if there is not a wildcard in the subdomain.

<5> A template can include multiple hosts, but Runtime Manager displays only the first path rule.
...
<1> A template can include multiple hosts, but Runtime Manager displays the only first path rule.

<2> These placeholder values are required for Kubernetes validation, but the actual values are not used by Runtime Fabric.
