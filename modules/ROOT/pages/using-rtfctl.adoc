...
The `rtfctl` command line tool enables you to locally manage Runtime Fabrics. Using `rtfctl` you can perform these actions:

* Install, configure, and troubleshoot Runtime Fabrics
* Get the status of a Runtime Fabric
* Perform a heap dump or a thread dump of a running application
* Display memory usage of a running Mule app
...
Before you start using `rtfctl`, see this topic to enable `rtfctl` Kubernetes permissions and install the `rtfctl` command line tool.
