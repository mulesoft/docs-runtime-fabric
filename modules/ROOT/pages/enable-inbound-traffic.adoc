...
+
Use the *Add Domain* button to specify domains to specify domains to use for routing requests to applications.
+
[NOTE]
...
+
The inbound load balancer runs on each controller node configured in Runtime Fabric. Refer to xref:deploy-resource-allocation.adoc[Resource Allocation and Performance on Anypoint Runtime Fabric] for more information.
...
*** *Dedicated Mode* 
+
Dedicated nodes are used in environments with high traffic demands. Refer to xref: and xref:deploy-resource-allocation#internal-load-balancer.adoc[Internal Load Balancer] for additional information.
+
...
... Optionally, if the *Key Passcode* field is displayed after uploading a PEM key file, enter the word or phrase that protects the private key.
