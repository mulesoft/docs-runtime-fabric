...
+
The number of inbound load balancer replicas is determined by the number of controller nodes configured in Runtime Fabric. A minimum of three replicas is required for production environments, and these replicas run only on the controller VMs. Refer to xref:manage-index.adoc[Manage Runtime Fabric] for detailed information.
+
*Shared Mode* is automatically selected for the ingress configuration if dedicated mode is not available.
...
... Specify a *Certificate File* in PEM format with the public certificate. This file contains the public certificate of the inbound traffic server. The *Derived URL Format* field lists domains that are selectable for *Application url*, which is used for routing requests to an application. By default, the first domain listed is used. Other values can be selected via the *Applications->Ingress* page.
+
