...
*** *Dedicated Mode* specifies that the entire node is available, so you cannot specify the amount of cores and memory. If *Dedicated Mode* is disabled, then all nodes are already in sharing mode, which prevents them from being dedicated.
+
[WARNING]
====
When switching between shared and dedicated mode:

* All traffic is lost if the shared or dedicated IP addresses are not included in the load balancer/DNS resolution 
prior to making the change.

* If you change from *Shared Mode* to *Dedicated Mode*, you must include the dedicated ingress node IPs in your load balancer in place of the controller node IPs. If you are setting up a Runtime Fabric with dedicated nodes, include 
only the dedicated node IPs.
