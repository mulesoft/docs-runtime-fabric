...
Runtime Fabric instances can be shared across multiple environments. After installation, configure one or more environments for each Runtime Fabric instance to enable application deployments. You must associate at least one environment with each Runtime Fabric instance to be able to deploy Mule applications or API gateways.
...
* Create or configure an external TCP load balancer to manage traffic to applications running in Runtime Fabric on VMs / Bare Metal:
...
+ 
The number of replicas for inbound traffic is set to the number of controller nodes for shared mode or the number of dedicated internal load balancer nodes for dedicated mode. You must include all IP addresses of the controller nodes or the dedicated internal load balancer nodes in your external load balancer or DNS resolution. Otherwise, traffic cannot be routed from the load balancer to Runtime Fabric. Refer to xref:deploy-resource-allocation.adoc#internal-load-balancer[Internal Load Balancer] and xref:index-vm-bare-metal.adoc#network-architecture[Network Architecture] for additional information.
+
...
. In the *Resource Allocation* section, select a *Shared Mode* or *Dedicated Mode*.
+
[WARNING]
====
...
+
Because certificates are not used in this case, click *Add Domain* and manually enter domains for routing requests to applications and skip the xref:configure-a-tls-context-for-hh[Configure a TLS context for HTTPS].
+
[NOTE]
...
. Still in Runtime Manager, select *Add certificate*.
. To configure a TLS context for HTTPs, choose the appropriate option between steps 3, 4, and 5. 
. Option 1: *Upload PEM*
+
...
.. In the *Select alias from keystore* window, which is enabled after you specify a keystore file and keystore passcode,
...
+
This option imports a TLS context from the secrets manager and supports advanced configurations such as creating a TLS context, mutual authentication, selecting ciphers, and selecting TLS versions.
+
Refer to the instructions in xref::configure-adv-tls-context.adoc[Import a TLS Context from Secrets Manager (Advanced)].

== (Optional) Select Security Policies

You must define security policies in Anypoint Security before they are displayed as options in the *HTTP Limits*, *Web Application Firewall (WAF)*, *IP Whitelist*, or *Denial of Service (DoS)* dropdown lists.
...
By default, the activity across all IPs addresses behind your endpoint is logged. To help reduce CPU consumption when using more verbose log levels, add IP filters to only log-specific IP addresses. This also reduces the quantity of logs when debugging a connection for a specific or limited number of IP addresses.
