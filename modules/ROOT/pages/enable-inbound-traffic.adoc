...
+
The number of inbound load balancer replicas is determined by the number of controller nodes configured in Runtime Fabric. A minimum of two replicas is required for production environments, and these replicas run only on the controller VMs. Refer to xref:manage-index.adoc[Manage Runtime Fabric] for detailed information.
+
*Shared Mode* is automatically selected for the ingress configuration if no dedicated mode is available.

.. Specify the minimum number of cores for each replica of the internal load balancer. Note that Runtime Fabric can use up to the maximum number shown if the cores are available. Transport Layer Security (TLS) termination is computationally expensive, so allocate enough CPU to increase throughput and decrease latency. Refer to See the xref:deploy-resource-allocation.adoc[Resource Allocation and Performance on Anypoint Runtime Fabric] for more information on CPU allocation and throughput, including how the TLS private key type and size affect these numbers.
...
... Specify a *Certificate File* in PEM format with the public certificate. The *Derived URL Format* field lists domains that are selectable for *Application url*, which is used for routing requests to an application. By default, the first domain listed is used. Other values can be selected via the *Applications->Ingress* page.
+
...
... Select *Deploy*. The *Key Passcode* field is blanked out for security reasons. You can still review public certificate details.
+
...
+
To generate key material for testing purposes and create a TLS context in the secrets manager, follow the instructions provided in xref:anypoint-security::secret-group-add-tls-context.adoc[Generate a TLS Certificate for Testing].
...
| The maximum number of requests per connections to allow. This value ranges from 1 to 4194304. The default value is 1000, which balances security and performance. Refer to xref:deploy-resource-allocation.adoc[Resource Allocation and Performance on Anypoint Runtime Fabric] for additional information.+
...
. Runtime Fabric is behind a non-AWS load balancer. +
 If Runtime Fabric is deployed behind another type of load balancer, such as F5 or nginx, the source IP address can be provided in an HTTP Header field. In this case, enter the HTTP header name that contains the source IP header. HTTP messages not containing this header field will be rejected. Two common HTTP header names that are used for source IP addresses are:
+
...
* A CN with a wildcard (such as *.example.com) uses the following request header format: `Host: {app-name}.example.com`. Use the following cURL command for verification:
+
----
curl -Lvk -XGET https://{ip-address}/{path} -H 'Host: {app-name}.example.com'
----

* A CN without a wildcard (such as example.com) uses the following request header format: `Host: example.com`. Use the following cURL command for vierfication:
+
----
...
== Generate a Certificate-Key Pair for Testing
...
* On the `Runtime Fabrics` page, when a TLS certificate has expired, a warning is displayed in the *Inbound traffic* column for that RTF.
