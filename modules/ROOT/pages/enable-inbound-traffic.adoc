...
Before enabling inbound traffic, you must provide a valid TLS certificate. The certificate must require a passphrase and be set with a Common Name (CN) which specifies the domain for each application deployed to Runtime Fabric.
...
.. Select *Users*
.. Locate the row with your username and then select the blue link.
...
.. Select *Add Keystore*.
+
If you don’t see the *Add Keystore* button, ensure that you are viewing your secret group in edit mode. Return to *Secret Groups* and then select the *Edit* next to your group.
...
. Select *TLS Context* and then perform the following:
+
[NOTE]
If you do not see *Add TLS Context*, ensure you are viewing your secret group in edit mode. Return to the *Secret Groups* page and then select *Edit* next to your group.
...
. After saving your TLS context, select *Secret Groups* and then select *Finish*. This enables TLS Context for your *Runtime Fabric.*
...
After creating a TLS context, apply the TLS context to your Runtime Fabric to enable secure incoming traffic.

. Navigate to Runtime Manager and then select *Runtime Fabric*.
...
. Select *Basic Configuration* and then perform the following:
