...
+
Use the *Add Domain* button to specify domains. Domains are used for routing requests to applications.
+
[NOTE]
...
*** *Dedicated Mode* 
+
Dedicated nodes are used in environments with high traffic demands. Refer to xref: and xref:deploy-resource-allocation#internal-load-balancer.adoc[Internal Load Balancer]
+
...
+
Refer to xref:deploy-resource-allocation#internal-load-balancer.adoc[Internal Load Balancer] and xref:architecture.adoc#network-architecture[Network Architecture] for additional information.
+
[WARNING]
====
...
... (Optional) If the *Key Passcode* field is displayed after uploading a PEM key file, enter the word or phrase that protects the private key.
...
... Use the *Details* button to view certificate details, including domains, expiration, and the other configuration settings.
...
... Use the *Details* button to view the information you entered for *Keystore File*, *Keystore Passcode*, *Alias*, and *Key Passcode* before selecting other options or deploying.
...
... Select the *…* button.
... Select *Delete*.
... To add updated certificate information:
