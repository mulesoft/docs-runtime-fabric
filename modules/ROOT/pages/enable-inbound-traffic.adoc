...
You’ll need a certificate-key pair for the internal load balancer in Runtime Fabric. If you have one already, both files must be of the .pem type, your keystore must have a passcode, and your Common Name should match the domain you’ll use in your Runtime Fabric. 
...
Use the following steps steps to generate a certificate-key pair for evaluation purposes.
...
* 2x or greater TLS 1.2 Connection Performance Throughput Boost.
* Reduction of one round trip in full handshake for TLS 1.3 vs. TLS 1.2.
* TLS 1.2 and TLS 1.3 ChaCha20-Poly1305 ciphers for better mobile and IoT device support.
* TLS 1.3 Protection against downgrade attacks.
* Continued support for both RSA and ECDSA keys.
* TLS 1.3 removes obsolete and insecure features from TLS 1.2 and TLS 1.1. Refer to xref:release-notes::runtime-fabric-release-notes.adoc[Anypoint Runtime Fabric Release Notes] for a complete list of supported and deprecated ciphers. 
