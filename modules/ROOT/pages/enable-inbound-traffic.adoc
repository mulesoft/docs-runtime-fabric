...
*** *Shared Mode*  - point to arch topic.

*** *Dedicated Mode*  point to arch topic??? If you are setting up a Runtime Fabric with dedicated internal load balancer nodes, include the IP addresses of the dedicated internal load balancer nodes only when configuring the external load balancer. Refer to xref:deploy-resource-allocation#internal-load-balancer.adoc for additional information????.
+
