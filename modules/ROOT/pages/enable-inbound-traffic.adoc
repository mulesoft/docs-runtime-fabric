...
** Option 1: *Upload PEM*
+
Use this option to upload a public certificate and private key in Privacy-Enhanced Mail (PEM) format and use TLS default values. With this option, you cannot change the default values for TLS versions, ciphers, and other TLS configuration options. A PEM file is a Base64-encoded ASCII file with a `.cer`, `.crt`, or `.pem` extension. Refer to xref:anypoint-security::configure-tls-context-rtf-lb.adoc[Configure a TLS Context for Runtime Fabric Load Balancer] for  information about TLS default values.
+
...
+
This option imports a TLS context from Secrets Manager, and supports advanced configuration such as creating a TLS context, mutual authentication, selecting ciphers, and selecting TLS versions. 
+
...
. Runtime Fabric is behind a non-AWS load balancer. +
 If Runtime Fabric is deployed behind another type of load balancer, such as F5 or nginx, the source IP header name must be provided. Two common source IP headers are:
+
