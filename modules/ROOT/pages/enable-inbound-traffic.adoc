...
The CorrelationID for Mule apps running in cluster mode on Runtime Fabric on VMs / Bare metal remains a valid UUID. This differs from other deployment targets where the CorrelationID is also prefixed with the cluster node ID.
...
+
NOTE: If you make any changes to the TLS context in Secrets Manager, they are not automatically applied to the inbound traffic configuration. If you make any changes, navigate to the *Inbound Traffic* tab, and click *Save and Deploy*.
...
This value enables requests with large payloads or slow clients to be terminated to keep resources available. This helps guard against connection pool exhaustion from slow requests or from clients who don't close connections after a response is sent.
