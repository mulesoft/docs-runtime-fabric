...
+
[NOTE]
If you are creating PEM or JKS certificates, You need to have the roles and permissions specified in <link to prereqs???????>
+
...
+
For HTTPS, you can add up to 10 certificates. You can configure a private key and public certificate for a TLS-enabled server in one of the following ways:
...
... Specify a value for *CA Path Certificate File (Optional)*. The CA path contains the intermediary and root certificates that provide the path (chain of trust) from the certificate to the root. When you provide an entry in this field, the path is downloaded from the CA when the certificate is signed.
...
Provision the external load balancer using round robin or other algorithm of your choice. The external load balancer routes traffic to the Runtime Fabric controller or dedicated nodes with the IPs identified during Runtime Fabric installation.
