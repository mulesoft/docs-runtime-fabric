...
+
*Shared Mode* is the default setting if no dedicated load balancer node is added in Runtime Fabric. In this mode, the internal load balancer runs on all controller nodes with the amount of CPU and memory specified. Dedicated mode specifies that the entire node is available, so you cannot choose the amount of cores and memory. If Dedicated mode is disabled, then all nodes are already in sharing mode, which prevents them from being dedicated.

.. Specify the minimum number of cores for each replica of the internal load balancer. When you select a Runtime Fabric from the Runtime Fabrics page, the sidebar that is displayed shows the total number of controller nodes deployed and total memory consumption for all deployed controller nodes. Use the sidebar information to help you specify values for the *Cores* and *Memory* fields for each replica. 
+
