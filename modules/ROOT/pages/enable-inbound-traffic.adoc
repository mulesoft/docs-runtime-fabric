...
The Correlation ID for Mule apps running in cluster mode on Runtime Fabric on VMs or bare metal remains a valid UUID. For other deployment targets, the Correlation ID is also prefixed with the cluster node ID.
