...
[IMPORTANT]
When deploying a mule application to Runtime Fabric through the Runtime Manager UI, the Runtime Manager always 
selects the first domain name that is listed in the certificate to use for calls made to deployer. 
Only this single domain or SAN is made active in the RTF.
