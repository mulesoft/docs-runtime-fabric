...
Make sure you have completed the prerequisite network-related tasks in xref:install-prereqs.adoc#infrastructure-requirements[Infrastructure Requirements] before enabling inbound traffic on Runtime Fabric.
...
. In the *Basic Configuration* section, select one of the following Protocol options. The Protocol option that you select determines how HTTP and HTTPS requests are handled:

* *Accept HTTPS requests…*
+
Select one of the following options:

** *and redirect HTTP to HTTPS*: After inbound traffic is enabled, HTTP requests sent to Runtime Fabric receive a 301 response to redirect to HTTPS on port 443.
** *and also accept HTTP requests*: HTTP requests sent to Runtime Fabric are directed to port 80.
** *and drop HTTP requests*: HTTP requests sent to Runtime Fabric are ignored.

* *Accept HTTP requests only*
+
Use the *Add Domain* button to specify domains. All specified domains are displayed in the *Domains* section.
+
[NOTE]
You can only change the TLS configuration and policies for Runtime Fabrics that you own. For inherited Runtime Fabric instances, the TLS configuration is read-only.

. In the *Resource Allocation* section:
+
The inbound load balancer runs on each controller node configured in Runtime Fabric. Refer to xref:deploy-resource-allocation.adoc[Resource Allocation and Performance on Anypoint Runtime Fabric] for detailed information.
...
+
If you selected *Accept HTTPS requests* in the *Basic Configuration* section, all inbound traffic entering Anypoint Runtime Fabric is encrypted using TLS. In this case, when enabling inbound traffic, you can provide up to 10 certificates, but at least one valid TLS certificate is required.
+
For HTTPS requests, you can add up to 10 certificates. You can configure a private key and public certificate for a TLS-enabled server in one of the following ways:
...
... Specify a value for *CA Path Certificate File (Optional)*. The CA path contains the intermediary and root certificates that provide the path (chain of trust) from the certificate to the root. The file is downloadable from the CA that signed your certificate. When you provide an entry in this field, the Runtime Fabric load balancer at every connection sends both the certificate and the path to the client. Many clients require the server to send the CA path so that the certificate can be validated.
... Review the certificate details to verify domains, expiration, and the other configuration settings. By default, the first domain listed is the domain that is used when deploying applications.
... Select *Deploy* after specifying any needed security policies or advanced options. The *Key Passcode* field is blanked out for security reasons. You can still review public certificate details. If you upload a new key file, this field is again enabled.
+
...
+
REFER TO OTHER TOPIC???This option imports a TLS context from the secrets manager, and supports advanced configuration such as creating a TLS context, mutual authentication, selecting ciphers, and selecting TLS versions. 
+
To import a TLS context from the secrets manager, you need additional secrets manager permissions. Refer to xref:anypoint-security::configure-tls-context-rtf-lb.adoc[Configure a TLS Context for Runtime Fabric Load Balancer] for additional information. To generate key material for testing purposes and create a TLS context in the secrets manager, follow the instructions provided.
...
+
The application's URL contains the routing path for the application. If the default domain is not the desired domain to be served by the application, navigate to the application's *Ingress* tab in Runtime Manager and select the desired domain from the *Domain* drop-down list.
...
Use the Ingress tab to update the configuration for application requests.


== View TLS Certificates

To view TLS certificate information:
...
== Replace or Update TLS Certificates

To update TLS certificate information, you must remove the certificate and then add a new one:
...
... Select *Delete*
... Select *Add*.
... Follow the instructions provided in Step 1 to configure a new certificate.
