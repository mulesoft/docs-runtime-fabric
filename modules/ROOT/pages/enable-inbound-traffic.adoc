...
+
*Shared Mode* is automatically selected for the ingress configuration if no dedicated mode is available.
...
** Option 1: Upload PEM
+
Use this option to upload a public certificate and private key in PEM format and use TLS default values. Refer to xref:anypoint-security::configure-tls-context-rtf-lb.adoc[Configure a TLS Context for Runtime Fabric Load Balancer] for  information about TLS default values.
...
After you enable inbound traffic, you must configure Runtime Fabric to route incoming traffic to each enabled application in order for clients to send requests to deployed applications. 
