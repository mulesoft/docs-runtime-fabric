...
+
[NOTE]
It can take up to 5 minutes for any permissions you add to propagate.
...
+
[NOTE]
The number of inbound load balancer replicas is determined by the number of controller nodes configured in the RTF. A minimum of 2 replicas is required for production environments, and these replicas only run on the controller VMs. Refer to xref:manage-index.adoc[Manage Runtime Fabric] for detailed information.

.. In the *Resource Allocation* section:
... *Shared Mode* is the default setting for the ingress configuration if no dedicated mode is available.
... Specify the minimum number of cores for each replica of the internal load balancer. Note that Runtime Fabric can use up to the maximum number shown if they are available. Transport Layer Security (TLS) termination is computationally expensive, so make sure to allocate enough CPU to increase throughput and decrease latency.
...
** Upload Privacy-Enhanced Mail (PEM) or Java Keystore (JKS) information. A PEM file is a Base64-encoded ASCII file with a `.cer`, `.crt`, or `.pem` extension. A JKS file is a repository for authorization or public key certificates and does not store secret keys. The default option is `Upload PEM`  when no TLS context exists for Runtime Fabric. When you select the *Upload PEM* or *Upload JKS* option, you cannot change the default values for TLS versions, ciphers, and other TLS configuration options. 
** Import a TLS Context from Secrets Manager.
+
This option supports advanced configuration such as mutual authentication, selecting ciphers, and selecting TLS versions. This option is automatically selected for Runtime Fabrics for which inbound traffic was enabled before the PEM and JKS upload options became available.
+
To import a TLS context from Secrets Manager, you need additional Secrets Manager permissions. Refer to xref:anypoint-security::configure-tls-context-rtf-lb.adoc[Configure a TLS Context for Runtime Fabric Load Balancer] for additional information.
+
...
+
This option uses the TLS default values. Refer to xref:anypoint-security::configure-tls-context-rtf-lb.adoc[Configure a TLS Context for Runtime Fabric Load Balancer] for additional information.
...
.... Select *Deploy*. The *Key Passcode* field is disabled. If you upload a new key file, the *Key Passcode* field is again enabled.
+
...
+
Select this option to create a TLS context or change TLS defaults, including adding or removing a TLS version, configuring mutual authentication, or changing cipher suites. This option is automatically selected for Runtime Fabrics that were configured prior to the availability of PEM and JKS uploads. Refer to xref:anypoint-security::configure-tls-context-rtf-lb.adoc[Configure a TLS Context for Runtime Fabric Load Balancer] for instructions.
...
+
If you do not see *Security* listed in *Management Center*, contact your customer success manager to enable Anypoint Security for your account.
+
Refer to xref:anypoint-security::index-policies[Anypoint Security Policies for Edge] for additional information.
...
* A CN with a wildcard (e.g. *.example.com) uses the following request header format: `Host: {app-name}.example.com`. Use the following cURL command for verification:
+
----
curl -Lvk -XGET https://{ip-address}/{path} -H 'Host: {app-name}.example.com'
----

* A CN without a wildcard (e.g. example.com) uses the following request header format: `Host: example.com`. Use the following cURL command for vierfication:
+
----
...
Runtime Fabric must be configured to route incoming traffic to each enabled application in order for clients to send requests to deployed applications. You must configure an external load balancer to load balance HTTPS traffic between each controller VM on Runtime Fabric.

Controller VMs are virtual machines dedicated to run the components that power Anypoint Runtime Fabric. Each controller VM runs a replica of the internal load balancer and is configured to listen on port 443. 
...
* On the *Runtime Fabrics* page, if a TLS certificate will expire within the next 30 days,`TLS Expiring` is displayed in the *Inbound traffic* column. On the `Runtime Fabrics` page, when a TLS certificate has expired, a warning is displayed in the *Inbound traffic* column for that RTF.
