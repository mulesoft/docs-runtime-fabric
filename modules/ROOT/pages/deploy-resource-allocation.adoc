...
[%header,cols="5*a"]
|===
| vCPU Cores | Max Requests per Second (Connection Re-use)* | Max Requests per Second (No Connection Re-use  with 2K RSA key)| Max Requests per Second (No Connection Re-use with 4K RSA key) |Max Requests per Second (No Connection Re-use with ECDSA key - curve p256)
| `1.00` | 2000 | 175 | |
| `0.75` | 1500 | 100 | |
| `0.50` | 1000 | 50 | |
| `0.25` | 100 | 10 | |
...
*Public key exchange is not applicable because it becomes a small part of CPU consumption.
