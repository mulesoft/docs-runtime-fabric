...
To ensure that you remain compliant with your MuleSoft license when using Runtime Fabric on VMs / Bare Metal, use the dasboard in Ops Center to determine your vCPU usage across all Mule applications.
...
* Runtime Fabric on VMs / Bare Metal runs a small amount of services on worker nodes. These services consume between 0.3 and 0.5 vCPU, depending on whether log forwarding is enabled for Anypoint Monitoring.
* Each application replica must reserve at least 50m of CPU and 50 Mi of memory for the `anypoint-monitoring` container. This container is used to stream application metrics to the control plane and cannot be disabled.
...
[%header,cols="3*a"]
.Minimum CPU resources required to support certificates
|===
