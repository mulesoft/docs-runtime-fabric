...
** `vCPU Limit`
+
The maximium amount of vCPU the application can use (the level to which it can burst). This is shared CPU on the worker node. 
* Memory
...
** The minimum reserve is 0.02 vCPU.
** The minimum limit is 0.07 vCPU. This aligns with the minimum for Runtime Fabric v1.3 and earlier.
** The CPU limit is bounded by the CPU cores provided on the worker nodes.
** The maximum suggested utilization per CPU core is 20 - 25 simple apps and API gateways. More complex apps that are frequently used should get more capacity. Specifying the minimum reserve and minimum limit provides approximately .5 cores to share across all apps when needed.
* Runtime Fabric runs a small number of services on worker nodes. These services consume between 0.3 and 0.5 cores, depending on whether log forwarding is enabled for Anypoint Monitoring.
* Minimize CPU contention. With CPU bursting, applications can compete for a shared resource: the unallocated CPU remaining on the worker node. As more apps are deployed on a worker node, each app has less unallocated CPU to share with other nodes, and apps can suffer from poor performance due to not getting enough CPU. The following strategies can be considered for minimizing CPU contention with your applications:

** Deploy intensive nightly batch applications with APIs that have peak load during the day.
...
+
Regardless of which approach you choose, make sure you run tests to validate performance.
...
* When sizing your worker nodes, make sure to keep your CPU usage in compliance with your MuleSoft license. MuleSoft licensing is based upon the amount of CPU cores that Mule apps and API gateways have access to use. 
