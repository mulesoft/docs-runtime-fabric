...
* The physical CPU core capacity is the CPU capacity provided by Runtime Fabric worker nodes.
* The MuleSoft license core amount is the entitlement needed for mule application deployment. 

For example, if you plan to have 1.5 cores available, you must purchase a MuleSoft license for 2 cores. You cannot purchase fractional MuleSoft license core amounts.

You are only required to purchase a license for the compute power you utilize. The underlying Runtime Fabric cluster does not need to be licensed. To measure utilization, use the dashboard in Ops Center. 

=== MuleSoft License Compliance

You will remain compliant with your MuleSoft license as long as Mule does not use more CPU cores than provided by your MuleSoft license at any given time.
