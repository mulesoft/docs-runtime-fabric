...
== Edge vCPU Allocation

Because Edge requirements are impacted by the number of threads, response time latency, and message sizes, the following recommendations are provided for Edge vCPU allocation:

* .5 GB (Default): Fewer than 500 simultaneous active connections.
* 1.5 GB (Large): One or both of the following scenarios:
...
The following Edge memory allocations are recommended:

* .5 GB: No security policies enabled and fewer than 150 continuous active requests. (Request size = .9 MB)
* 1.0 GB: Security policies enabled and fewer than 250 active requests. (Request size = .9 MB)
* 2.0 GB: Security policies enabled and greater than 250 active requests. (Request size .9 MB)

[NOTE]
Requests with sizes greater than 1MB are streamed and do not use memory to buffer the entire request.
