...
* vCPU cores
** `Reserved vCPU` - the amount of vCPU guaranteed to the application and reserved for its use.
** `vCPU Limit` - the maximium amount of vCPU the application can use (the level to which it can burst). This is shared CPU on the worker node. 
* Memory

== vCPU Allocation

When the Reserved vCPU and vCPU limit are equal, the CPU on the worker node is allocated in a guaranteed model. The container always has the specified amount of CPU available. The vCPU is reserved for each replica of the application and is guaranteed regardless of what else is running on the Runtime Fabric.

Guaranteed CPU allocation allows applications to perform consistently by ensuring there is no CPU contention between other applications. It gives guaranteed performance in all cases.

When the `vCPU Limit` is set to a higher value than the `Reserved vCPU`, the application can burst up to the either the vCPU limit value or the amount of unreserved vCPU on the worker node, which ever is lesser.
...
* Memory: 0.7 GiB memory (Mule 4) or 0.5 GiB (Mule 3)
