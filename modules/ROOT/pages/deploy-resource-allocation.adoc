...
The resources allocated to your Mule application determine the application's performance. Below are the approximate values for throughput based on the amount of vCPU cores allocated for a single Mule application performing simple processing on a 10 KB payload.
...
Inbound traffic is processed using an internal load balancer managed by Anypoint Runtime Fabric. Since this load balancer is responsible for TLS termination, the number of resources required scales based on the number of incoming connections and the average payload size for each request.

Performance tests were run on a Runtime Fabric cluster with controller nodes using AWS EC2 M4 instances. The load generator that was used was hosted on a separate AWS instance in the same region. The M4 instances featured a custom Intel Xeon E5-2676 v3 Haswell processor optimized specifically for EC2, which runs at a base clock rate of 2.4 GHz. Using Intel Turbo Boost, the clock rate can go as high as 3.0.
...
[NOTE]
The internal load balancer runs on the controller VMs of Runtime Fabric. Size the VMs appropriately based on the amount and type of inbound traffic. You can allocate only half of the available CPU cores on each VM to the internal load balancer.
