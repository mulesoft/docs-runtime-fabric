...
Resumed upgrades are attached to your terminal session. Ensure you’re on a stable connection before attempting to resume an upgrade.

. Resume the upgrade from where it stopped on the controller node used to start the upgrade: 
.. On a terminal open to the controller node performing the upgrade, change to the directory with the installer bundle files:
+
----
cd /tmp/rtf-upgrade
----
...
If the error occurs again, follow the troubleshooting steps.
