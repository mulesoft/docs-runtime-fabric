...
For Runtime Fabric on VMs / Bare Metal, the `rtfctl` binary is in directory `/opt/anypoint/runtimefabric/` and the current working directory is also `/opt/anypoint/runtimefabric/`.

For Runtime Fabric on Self-Managed Kubernetes, confirm that the `rtfctl` binary is present in the current directory and in the user `$PATH`.   
