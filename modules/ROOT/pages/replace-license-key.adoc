...
. On the controller node acting as the leader during installation (the installer node), run the `rtfctl` command with the
base64 value of your license key. For more information on the `rtfctl` command, refer to xref:runtime-fabric::install-rtfctl#supported-commands[rtfctl Supported commands].
