...
* The necessary ports are open and hostnames are added to allowlists as described in xref:install-prereqs.adoc[Anypoint Runtime Fabric on VMs / Bare Metal Installation Prerequisites].
...
. Familiarize yourself with the xref:variables-to-define[variables needed for installation]. 
. Use the `./generate-configs.sh` script to generate the environment variables for each VM.
. Copy the command below in a text editor and fill in the values of each variable inside the single quotes. Add and define any xref:optional-variables[optional variables] if required. A common example is if you require an HTTP proxy for outbound internet connectivity.
+
----
RTF_CONTROLLER_IPS='' \
RTF_WORKER_IPS='' \
...
Use the following required variables to install Rutime Fabric. 
...
=======
| `RTF_NO_PROXY` | A comma-separated list of hosts which should bypass the proxy. Runtime Fabric looks for upper case values for the hosts, but if upper case values are missing, Runtime Fabric will pick lower case values. If both values are present, Runtime Fabric picks the upper case values. | `1.1.1.1,no-proxy.com`
