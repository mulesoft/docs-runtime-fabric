...
If you use AM log forwarding, you must additionally perform these steps:

. Enable daemon-set log forwarding by calling this API (bearer token required):
+
----
...
[NOTE]
Mule apps redeployed while Runtime Fabric agent version was 2.x must be redeployed to reduce the resource allocation per the downgraded rtf-agent version.