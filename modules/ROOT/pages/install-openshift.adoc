...
You can install Anypoint Runtime Fabric as a Kubernetes (K8s) operator in these platforms:
...
* You have access to these URLs needed for Red Hat OpenShift registry:
...
Before configuring authorized namespaces, note this:
...
. In your cluster, create an additional namespace for application deployments, and add the necessary labels to the namespace. To do so, create a YAML file with these contents:
+
[source,copy]
----
apiVersion: v1
...
. Create the RoleBinding for the Runtime Fabric agent ClusterRole that includes the Runtime Fabric agent ServiceAccount. To do so, apply this configuration in your additional namespace:
+
[source,copy]
----
kind: RoleBinding
...
# If using persistence gateway uncomment and provide this. Ref: https://docs.mulesoft.com/runtime-fabric/latest/persistence-gateway#persistence-gateway-with-authorized-namespaces
# subjects:
#  - kind: ServiceAccount
...
. Apply this RoleBinding template to rtf namespace and any additional namespaces:
+
[source,copy]
----
apiVersion: rbac.authorization.k8s.io/v1
...
[NOTE]
====
Starting with Runtime Fabric agent version 3.0.0, the `am-log-forwarder` service account is no longer required as log forwarding occurs exclusively through application sidecar containers. You can omit this service account for new installations of version 3.0.0 or later.
====

To use clustered apps in authorized namespaces mode, this RoleBinding must be created:
...
. If, after fully installing Runtime Fabric, you later add or delete any namespaces from the ConfigMap, you must restart the Runtime Fabric agent pod. To do so, run this command:
+
[source,copy]
----
...
global:
  ...
  image:
    rtfRegistry: <rtf-runtime-registry.kprod.msap.io or local registry value>
...
----
+
* On Windows, choose one of these:
...
To apply the license using Helm, run this command:
...
Before you deploy any Mule applications to your Red Hat OpenShift cluster, ensure that you've installed your Mule license key and then, perform these steps.
