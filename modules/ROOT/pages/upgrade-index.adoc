...
It is important to run with the most current versions of the appliance and `rtfctl` command-line utility.
...
.. If the amount of available resources for application deployments is less than the resources of one worker node, add another worker node with the same amount of CPU, memory, and disk resources to the Runtime Fabric.
+
...
+
In most cases, the upgrade takes less than 10 minutes. When the status transitions to *Active*, and the version of the Runtime Fabric reflects the new version, the component upgrade is complete.
+
...
+
[IMPORTANT]
To upgrade to Runtime Fabric version 1.5 or later, you must be running version 1.4.54. If you are not running version 1.4.54, you'll be shown a button to upgrade to 1.4.54. Upgrade to this version in order to enable upgrades to 1.5.
...
*** The internal load balancer must be running on at least 3 replicas to maintain availability. On newer versions of Runtime Fabric, the internal load balancer runs on all controller nodes by default.
...
... Run `/opt/anypoint/runtimefabric/rtfctl status` and confirm the cluster status is healthy.
