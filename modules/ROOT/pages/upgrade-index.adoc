...
Anypoint Runtime Fabric is a package of components that work together, and updates for all appliance components are obtained using Anypoint Runtime Manager.

* Appliance components
Appliance components include:

** Cluster-level components, which include Docker, Kubernetes, and the Ops Center. These components run on virtual machines or bare-metal servers that act as nodes on the cluster. Cluster-level components are also referred to as appliance components.
+
Cluster-level component upgrades are delivered as an updated installer package via the *Runtime Fabric downloads* page.

** Runtime Fabric components, which include the agent used to connect Runtime Fabric to Anypoint Platform.
+
Runtime Fabric updates are delivered via a link to the downloads page in the *Upgrade Available* and *Appliance Version* fields, as applicable.
...
+
Upgrades are delivered as a new binary via the *Runtime Fabric downloads* page.
...
+
Changes are delivered as an updated install script version via the *Runtime Fabric downloads* page.
+
[NOTE]
...
== Determine Each Component Version You Are Using
...
+
[NOTE]
The Runtime Fabric component version and the cluster-level component (appliance) version are also displayed in the *Runtime Fabrics* tab in Runtime Manager. If an upgrade is available, "Upgrade to vx.x" is displayed.
...
+
In most cases, the upgrade takes fewer than 10 minutes. When the status transitions to Active, and the version of the Runtime Fabric reflects the new version, the upgrade is complete.
+
