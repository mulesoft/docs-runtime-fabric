...
To upgrade the `rtfctl` command-line utility:
...
* If you've customized Runtime Fabric email alerts, the appliance upgrade overwrites the customizations. Refer to the "Appliance Upgrades" section of https://help.mulesoft.com/s/article/How-to-Modify-Runtime-Fabric-Email-Alert-Contents[How to Modify Runtime Fabric Email Alert Contents^] from the MuleSoft Knowledge Base for a workaround of this scenario.
