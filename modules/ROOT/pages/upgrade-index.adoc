...
Anypoint Runtime Fabric contains a collection of services that run within the cluster, including the agent used to connect to Anypoint Platform. ???

* Cluster-level components are delivered as an updated installer package, which includes Docker, Kubernetes, and the Ops Center, via the *Runtime Fabric downloads* page in Anypoint Runtime Manager. The installer package is also referred to as the Runtime Fabric appliance on the *Runtime Fabric downloads* page in Anypoint Runtime Manager and in `rtfctl` output.

* Runtime Fabric components are delivered via the Runtime Fabric page in Anypoint Runtime Manager. These components include the agent used to connect Runtime Fabric to Anypoint Platform. If an updated application or appliance version is available, a link to the downloads page is displayed in the *Upgrade Available* and *Appliance Version* fields, as applicable.

* Node configuration changes are reflected in newer install script versions and are delivered via the *Runtime Fabric downloads* page in Anypoint Runtime Manager, and are also delivered via the `rtfctl` command-line utility. These changes include adjusting operating system limits and performing jobs to clean up system resources.

* The `rtfctl` command-line utility upgrades are provided as a new binary that replaces the existing binary, and are delivered via the *Runtime Fabric downloads* page in Anypoint Runtime Manager. The `rtfctl` utility is used to manage Runtime Fabric locally.
+
The `rtfctl` command-line utility is used to perform the actual upgrades to cluster-level and node configuration components.
...
If an installer package upgrade is available, a code snippet for performing the upgrade is provided, as shown in the following example:
```
...
=== Availability of Deployed Applications for cluster?????
...
.. If an upgrade is available, select "Upgrade to vx.x". The status of the Runtime Fabric is updated to reflect the upgrade procedure is in progress.
