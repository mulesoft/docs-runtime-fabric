...
Anypoint Runtime Fabric components work together but are upgraded using different mechanisms:

* Appliance Upgrades
+
** Appliance upgrades impact platform layer components, which include Docker, Kubernetes, and the Ops Center. These platform layer components run on virtual machines (VMs) or bare-metal servers that act as nodes on the cluster.
+
Appliance upgrades are applied using the `rtfctl` command line utility.
+
Appliance upgrades are applied as a rolling upgrade to each node, one at a time. When taking each node offline for the upgrade, one worker node's amount of resources are required to transfer workloads and prevent downtime to running applications.

* Runtime Fabric Component Upgrades

** Runtime Fabric component upgrades involve the internal components that facilitate communication to the Anypoint control plane, provide load balancing, and log forwarding.
+
Runtime Fabric component upgrades are applied by selecting a link that is provided in Runtime Manager. 
+
Runtime Fabric component upgrades do not require adding a node.

* `rtfctl` Command-line Utility Upgrades

** The `rtfctl` command-line utility, is used to manage Runtime Fabric and perform appliance and node conifguration upgrades.
+
...
It is important to run with the most current versions of the appliance (installer package) and `rtfctl` command-line utility.
...
sudo chmod +x rtfctl
----
. If you are not using the latest version of the appliance (installer package), upgrade to the latest version:
