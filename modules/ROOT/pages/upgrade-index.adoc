...
* Review https://help.salesforce.com/s/articleView?id=001116597&type=1[What Items to Check Before an RTF Appliance Upgrade^] from the MuleSoft Knowledge Base. 
...
[NOTE]
--
When you run the `apply system-configuration` command on worker nodes, you might see an error similar to the following:

`Warning: Agent pod is not running.Warning: Failed to get agent version from chartWarning: Failed to get agent version from podDownloading latest scripts`
...
See https://help.salesforce.com/s/articleView?id=001116586&type=1[How to Troubleshoot RTF Appliance Upgrade] for additional troubleshooting information.
