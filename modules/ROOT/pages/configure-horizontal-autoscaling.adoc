...
* Max scale up profile is 1 -> 2 -> 4 -> 8, where MuleSoft hits 8 replicas in approximately 9 minutes.
...
You can track the scaled up replicas startup and the number of replicas of your Mule apps by running the following `kubectl` command in your terminal:
