...
These are recommendations, the actual configuration of the cluster autoscaler (CA) will depend on the cloud/infrastructre provider (vendor) being used and the topolgy considerations applicable.  Properly configuring the CA is customer managed responsibility.
...
The recommendation of using `whenUnsatisfiable: DoNotSchedule` is to ensure pods are evenly spread across availability zones. To change this setting you have to make use of the K8S templates CRD to change your deployments using HPA to `whenUnsatisfiable: DoNotSchedule`. You can use dedicated namespaces (environments) to separate the autoscaling workloads from the classic ones.
If you don’t make this change you will end up using `whenUnsatisfiable: ScheduleAnyway`.
...
This means that HPA bases on https://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale/#how-does-a-horizontalpodautoscaler-work[resource requests].
[quote,kubernetes.io]
----
...
----
If pod average utilization is greater than 70% in a period (periods for scale up are set as 180 seconds) HPA will scale up. Since this is a Kubernetes feature please refer to that documentation for more details.
