...
The controller nodes are the entry points into your Runtime Fabric cluster but do not run application workloads.

To update controller nodes:

. On your upstream TCP load balancer, remove the controller nodes one at a time from your load balancing pool.
...
```
+
If you have multiple environments, repeat the remaining steps in each environment. 
+
. Perform the following steps on all worker nodes that require updates:
...
+
[IMPORTANT]
Unless you are running at least 2 replicas of your application and specified *Enforce deploying replicas across node* when you deployed it, service is disrupted during the drain process.
+
```
