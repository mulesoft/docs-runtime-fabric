...
. Validate that you Kubernetes cluster is read for installation:
+
----
...
+
The `rtfctl` command outputs any incompatibilities with Kubernetes cluster.
...
When the `rtfctl` command finishes, it outputs confirmation that the Runtime Fabric is ready.
