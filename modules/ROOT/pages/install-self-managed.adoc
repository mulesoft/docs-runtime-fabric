...
[IMPORTANT]
====
Installing Runtime Fabric on Self-managed Kubernetes requires tasks that must be peformed by both IT administrators and MuleSoft organization administrators.
====

Anypoint Runtime Fabric supports installing and running Runtime Fabric in a Kubernetes environment on AWS Elastic Kubernetes Service (EKS) or Azure Kubernetes Service (AKS) that you manage.
...
== Step 1: Configure your Network to Support RTF
...
== Step 2: Download the rtfctl Utility
...
== Step 3: Create a Runtime Fabric using Runtime Manager
...
== Step 5: Update the Mule License Key
