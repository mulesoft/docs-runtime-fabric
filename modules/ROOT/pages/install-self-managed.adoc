...
Refer to xref:rtf-scale.adoc[scalability benchmarking] to review the limits for `rtfctl` based deployments. For larger deployments, use Helm based Runtime Fabric deployments so additional resources can be allocated to the agent for optimal performance.
