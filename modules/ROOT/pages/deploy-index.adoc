...
Use one of these options to deploy Mule apps to Runtime Fabric:
...
When an application is deployed, these events occur:
...
If an application fails, for example, due to running of out memory, these events occur:
...
* If the replica is initially unable to restart, for example, it relies on a network resource which temporarily unavailable, these events occur:
