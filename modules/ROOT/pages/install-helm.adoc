...
=== Core Software High Availability Parameters

From Runtime Fabric version 2.11.0 onwards, high availability parameters can be configured.

Set these optional parameters in `values.yml`  to configure the Runtime Fabric Core Software High Availability parameters.
...
For each runtime fabric deployments (`agent`, `resource-cache` and `mule-clusterip-service`) resources and amount of replicas can be configured.

In the case of the `agent` deployments, multiple replicas will not increase the throughput it is capable of handling, but in the case of an issue with the leading pod, one of the remaining replicas will take the lead and serve traffic.

By modifying the `topologySpreadConstraints` object it is now possible to determine how each deployment replicas will be spawned among the cluster node. Default value used is the one shown in the example

[NOTE]
--
If these values are set but the Runtime Fabric instance is later rolled back to a previous version, the configuration will not be applied for the older version.
--
...
== Retrieve Helm Registry Endpoints and Credentials
...
`https://<ANYPOINT_PLATFORM_URL>/runtimefabric/api/organizations/<ORG_ID>/helmrepoproperties`
...
For example:
`rtf-runtime-registry.kprod.msap.io/charts`
