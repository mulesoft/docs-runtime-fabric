...
If you want to configure xref:authorized-namespaces.adoc[authorized namespaces], add the required Kubernetes objects. The objects include:
...
[NOTE]
To configure authorized namespaces, Runtime Fabric has clusterwide permissions for the rtf:resource-metrics-collector clusterRole. These permissions apply to all nodes, pods, and namespaces.
...
+
[IMPORTANT]
You must name the ConfigMap `authorized-namespaces`.
