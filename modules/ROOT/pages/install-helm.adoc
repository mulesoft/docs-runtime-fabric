...
If you want to configure xref:authorized-namespaces.adoc[authorized namespaces], add the required Kubernetes objects. 

[NOTE]
Runtime Fabric has cluster-wide permissions for the rtf:resource-metrics-collector clusterRole. These permissions apply to all nodes, pods, and namespaces.
...
metadata:
  ...
  namespace: {{ RTF_NAMESPACE }}
data:
  ADDITIONAL_NAMESPACE: "additional-namespace"
----
+
[IMPORTANT]
You must name the ConfigMap `authorized-namespaces`, and you must create it _before_ installing Runtime Fabric.
...
[source,copy]
----
kubectl delete pod <agent-pod-name> -n <agent-namespace-name>
----
+
