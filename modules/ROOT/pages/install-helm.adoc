...
To obtain the Helm registry endpoints and credentials, make a request to this API endpoint using a bearer token for authentication. Replace ORG_ID with your organization's ID, and ANYPOINT_PLATFORM_URL with your Anypoint Platform region URL:
...
The URL for the Helm repository containing the Helm charts, along with the registry endpoints and credentials, is constructed as follows:
