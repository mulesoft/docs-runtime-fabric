...
. In the `rtf` namespace, create a ConfigMap file named `rtf-authorized-namespaces` and list any additional namespaces:
+
[source,copy]
----
apiVersion: v1
...
+
[IMPORTANT]
You must name the ConfigMap `rtf-authorized-namespaces`.
...
|===
|Key |Value |Example
|`sharedTenancy` |Enables shared tenancy |`sharedTenancy=true`
a|      
