...
* You have installed Helm 3 or later.

[NOTE]
--
If you use Helm to manage your Runtime Fabric installation, you can still later use the `rtfctl` command line utility to manage Mule license keys and proxies. However, you’ll need to track any post-installation changes you make, as explained in the upgrade documentation [add link!!!]. 
--

== Create a Runtime Fabric Using Runtime Manager

To install Runtime Fabric on Self-Managed Kubernetes with Helm, first create a Runtime Fabric using Runtime Manager. This is required to obtain the activation data which is needed during installation.
...
. Click Helm.
. Note that your activation data is captured in the code example provided by Runtime Manager. You will need this activation data to install Runtime Fabric.
...
After you create the namespace, you create a pull secret so you can retrieve the Docker images needed to install and run Runtime Fabric. 
...
To install Runtime Fabric, you use the `values.yml` file supplied by Runtime Manager. 
