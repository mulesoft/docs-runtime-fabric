...
Before installing or using Runtime Fabric, ensure that these ports and hostnames are configured correctly.
Note that all internal communication within the cluster (between pods and services) needs to be allowed.

== Port Configuration

To install or run Runtime Fabric, ensure that you have configured these ports on your Kubernetes installation.

For protocol-aware firewalls, consider allowing this port traffic to use AMQP (Advanced Message Queuing Protocol) over Websockets.
...
To function correctly, Runtime Fabric requires these hostname endpoint configurations. Give access to these endpoints in your infrastructure to avoid any installation or deployment issues.
...
If you experience any issues, refer to https://help.salesforce.com/s/articleView?id=001119347&type=1[ERROR in sidecar logs] knowledge base article.
...
To verify the required outbound connectivity, run this command on all controller and worker nodes in the cluster:
...
To allow different endpoints to use mutual TLS authentication to establish a connection, you must configure SSL passthrough to allow these certificates:
