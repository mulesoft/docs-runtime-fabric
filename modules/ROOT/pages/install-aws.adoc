= Installing Runtime Fabric on AWS
...
Before running Terraform, modify the `fabric.tf` script to reflect your specific environment. The following tables
...
| `private_interface` | Allows private IP address to be discovered from the network interface. | `eth0`
| `custom_log4j_enabled` | Enables or disables custom Log4j configurtaions. Set to `false` by default. | `false`
|===
