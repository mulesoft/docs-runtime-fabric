...
The installer VM downloads the installer package, unpack it and begin installation. The other VMs waits for the installer VM to progress with installation until it can make the installer files transferrable. Each VM then transfers the files from the installer VM and carry out their own installation procedure.

When installation has been completed, a cluster is formed across all VMs. The installer VM then completes the registration step using the Anypoint Organization ID, token, and region specified.

After registration has completed, Runtime Fabric appears in Anypoint Runtime Manager, under the Runtime Fabrics tab. The installation script on the installer VM  inserts the Mule Enterprise license digest in Runtime Fabric.
