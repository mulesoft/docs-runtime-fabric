...
| `disable_selinux` | A boolean variable that disables/enables SELinux on the host operating system. The default value is `true` to disable SELinux.
{sp} +
{sp} +
If the host operating system is RHEL 8, do not set to `false` or installation will fail. | `true`
...
=======
| `no_proxy` | A comma-separated list of hosts which should bypass the proxy. Runtime Fabric looks for upper case values for the hosts, but if upper case values are missing, Runtime Fabric will pick lower case values. If both values are present, Runtime Fabric picks the upper case values. | `1.1.1.1,no-proxy.com`
>>>>>>> parent of fc04d56 (clarify no_proxy behavior)
