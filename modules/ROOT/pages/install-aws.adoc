...
In Runtime Fabric, the inbound load balancer runs on each controller node in either shared or dedicated mode: 

*Shared Mode* is the default setting and specifies the internal load balancer runs with the amount of CPU cores and memory you specify.
*Dedicated Mode* specifies that the entire node is available, so you cannot specify the amount of CPU cores and memory.
