...
| `activation_data` | Specifies the encrypted Runtime Fabric activation data | NzdlMzU1YTktMzAxMC00OGE0LWJlMGQtMDdxxxx
...
This step runs the Terraform script to provision the infrastructure and run the installation script on each VM.
The installer VM downloads the installer package, unpacks it, and begins installation. The other VMs wait for the installer VM to progress with installation until it can make the installer files transferrable. Each VM then transfers the files from the installer VM and carries out the installation procedure.

When installation is complete, a cluster is formed across all VMs. The installer VM then completes the registration step using the Anypoint Organization ID, token, and region specified.

After registration is complete, Runtime Fabric appears in Anypoint Runtime Manager, under the Runtime Fabrics tab. The installation script on the installer VM  inserts the Mule Enterprise license digest in Runtime Fabric.
...
The `cloud-init` library is used to execute the Runtime Fabric installation script. It's useful to monitor the installation on the installer VM to verify all pre-flight checks have passed. If an error occurs during installation, it's likely to be visible by viewing the installer VM's log output.
