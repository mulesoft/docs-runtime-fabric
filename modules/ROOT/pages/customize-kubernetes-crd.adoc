...
== Enable Readiness Probe on HTTP GET

To enable the native K8s HTTP GET probe for readiness on Mule applications using the Kubernetes template, set the `ENABLE_READINESS_PROBE_ON_HTTP_GET` variable in the `env` section:
