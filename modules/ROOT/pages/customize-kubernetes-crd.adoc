...
The `kubernetestemplate` resource enables you to configure these elements of your apps pods, deployments, and services objects:
...
The env supported only keywords are for <<enable-console-logging, enable console logging>> and <<disable-service-account, disable service account>>.
...
To get the CRD of key elements that you can configure for the template, you can get the Kubernetes specification by running this command:
...
include::partial$log-console-stdout.adoc[]
...
By default, a https://kubernetes.io/docs/concepts/security/service-accounts/[service account] is created for every Mule application deployment. You can override this default behavior using the Kubernetes template, either at the cluster level with the root Kubernetes template or at the Mule app namespace level. 
If disabled, the Mule application deployment uses the https://kubernetes.io/docs/concepts/security/service-accounts/#default-service-accounts[default service account].
