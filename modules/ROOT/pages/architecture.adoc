...
Anypoint Runtime Fabric is composed of a set of VMs that form a cluster. Each VM serves are either a controller or a worker.
...
Anypoint Runtime Fabric supports development and production configurations. These supported configurations specify the minmum VMs required.

=== Development Configuration

The development configuration is intended for testing only. It requires at least one controller and two worker nodes. The controller VM runs the internal load balancer and agents used to connect to Anypoint Platform. Communication between the agent and Anypoint platform is always outbound. Multiple replicas of application can run across workers.
