...
[NOTE]
Make sure to provision an odd number of controller nodes, because an odd number of controller nodes is required for fault tolerance.

+
Consider scaling the number of controller nodes in the following situations:

*** Fault tolerance is desired to mitigate hardware failure on a controller node.
