...
|===
| Port | Layer 4 Protocol | Layer 5 Proocol | Source | Destination | Description
| 61008-61010 | TCP | HTTPS | All nodes | All nodes | Used during installation
| 61022-61024 | TCP | HTTPS | All nodes | All nodes | Installer agent ports
| 4242 | TCP | ? | All nodes | All nodes | Bandwidth checker
|===

=== Network TCP and UDP Ports

The following table lists all the ports that must be open, along with the respective protocols used and the request origin and destination:
...
| 7496 | UDP | HTTPs | All nodes | All nodes | Serf (healh checking agent) / Peer-to-peer connectivity
...
In addition to the port requirements above, the following network settings are required to operate Anypoint Runtime Fabric:
