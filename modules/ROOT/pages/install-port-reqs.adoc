...
Runtime Fabric requires specific network and port configuration for installation and operation.

== Bandwidth Requirements

A high bandwidth network is required, with a minimum of 1 GBps connectivity in the internal network running the RTF nodes. 
[NOTE]
Due to data center connectivity being lower than the minimum requirement, cross-regional deployments of a Runtime Fabric 
is not supported. Latency can cause problems with cluster health.
...
The table below lists the ports that must be accessible for Anypoint Runtime Fabric installation. After completing the installation, you can safely disable these ports.
...
The table below lists all the ports that must be open, along with the respective protocols used and the request origin and destination:
...
| 3008 - 3012 | TCP | HTTPS / gRPC | All nodes | All nodes | Runtime Fabric internal services
| 3022 - 3025 | TCP | SSH | All nodes | All nodes | Internal SSH control services
| 3080 | TCP | HTTPS | All nodes | Controller nodes | Runtime Fabric internal Ops Center
| 5000 | TCP | HTTPS | All nodes | Controller nodes | Internal container registry
| 6060  | TCP | HTTPS / gRPC | All nodes | All nodes | Runtime Fabric internal services
...
| 32009 | TCP | HTTPS | Internal network | Controller nodes | Runtime Fabric Ops Center
...
The table below lists the ports and hostnames to add to your whitelists to allow communication between Runtime Fabric and Anypoint Platform:
...
In addition to the port requirements above, these network settings are required to operate Anypoint Runtime Fabric:

* A subnet with enough IP addresses allotable to add additional VMs to Runtime Fabric.
...
* Access to an SMTP server to manage e-mail alerts triggered by Runtime Fabric.
