...
[NOTE]
Cross-regional deployments of Anypoint Runtime Fabric are not supported. Latency can cause problems with cluster health.
...
The following table lists the ports that must be accessible for Anypoint Runtime Fabric installation. After completing the installation, you can safely disable these ports.
