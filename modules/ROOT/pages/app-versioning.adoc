...
Watch the following video to see how to review the versioning of a Mule application deployed to Runtime Fabric by using Runtime Manager:

++++
include::_partials/rtf-app-versioning-video.html[]
++++
...
+
image::app-versioning-replica-status.png["Replicas status for a Mule app in the Runtime Manager UI"]
+
Once the configuration is successfully applied and deployed, a message appears at the top of the page.
+
image::app-versioning-config-applied-success.png["Configuration applied successfully status for a Mule app in the Runtime Manager UI"]
+
...
+
image::app-versioning-update-failed.png["Configuration failed to apply status for a Mule app in the Runtime Manager UI"]
+
