...
When an appliction is deployed to Runtime Fabric, Runtime Manager automatically versions the application and its configuration. Applicationg versioning enables you to:
...
+
Runtime Fabric displays each change to the application configuration that is successfully deployed to in the *Config changes* tab on the applications *Settings* page.
+
The last 10 configuration deployments are displayed in order of their respective configuration date. Each configuration in the history has a unique hash to identify it.
