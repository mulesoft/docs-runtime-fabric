...
If you are upgrading from Runtime Fabric agent version 2.11.x or earlier and currently use the log forwarder DaemonSet, you must complete migration steps before upgrading. See xref:upgrade-pre-requisites.adoc#migrate-log-forwarding-3-0-0[Migrate Log Forwarding Before Upgrading to Runtime Fabric Agent Version 3.0.0].
...
For instructions on enabling trace data collection for Mule apps deployed to Runtime Fabric, see xref:manage-monitor-applications.adoc#enable-trace-data[Enable Trace Data Collection].
