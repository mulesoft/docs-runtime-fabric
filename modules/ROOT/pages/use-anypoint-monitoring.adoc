...
To verify outbound network connectivity on Anypoint Runtime Fabric version 1.8.50 or later, ensure that each node in Runtime Fabric can send outbound TCP (Lumberjack) communication using port 443 to these domains:
...
[NOTE]
====
If you are using a version of Anypoint Runtime Fabric older than 1.8.50, use port 5044 instead of port 443. Use these domains:
...
Starting with Runtime Fabric agent version 2.11.x, application logs are forwarded exclusively through a sidecar container deployed alongside each application replica. This architecture provides better isolation, improved resource management, and eliminates the need for node-level log forwarding agents.
...
Monitoring, pass this custom property when deploying your Mule app:
