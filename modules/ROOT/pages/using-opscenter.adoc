...
. SSH into one of the nodes of the Runtime Fabric installation.
. From the SSH terminal, run the following command to create a new user with admin rights to log into the Ops Center. Provide the appropriate values for `--email` and `--password`.
----
...
The dashboad should populate the metrics for your application. By default, Ops Center displays a set of graphs showing the CPU, JVM Heap Memory, Total Memory, Network, and Filesystem usage.
