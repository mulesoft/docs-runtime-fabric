...
The `rtfctl agent-truststore` command group lets you inspect and update the CA certificates that the Runtime Fabric agent uses to establish mutually authenticated TLS (mTLS) connections with the Anypoint control plane.

== About the Agent Truststore

The Runtime Fabric agent communicates outbound with several MuleSoft services hosted in the Anypoint control plane:
...
== Why Manage the Truststore

The Anypoint control plane is backed by public CA root certificates, which are inherently long-lived and stable. Rotation of these certificates is infrequent. However, in cases where a rotation is required for security reasons — such as a CA compromise or a planned infrastructure change — you must add the new CA to the agent's truststore before the old one is retired to maintain the mTLS connection between the Runtime Fabric agent and the control plane.
...
=== Add a New CA Manually (Step by Step)
