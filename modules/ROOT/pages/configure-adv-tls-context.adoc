...
If your environment has specific security requirements or requires advanced configuration, you can import a TLS context from the secrets manager.
...
If you do not have access to *Access Management*, ask your organization admin (system administrator) to provide the "Manage Runtime Fabrics" permission.
...
.. Select *Environment* and then select your environment from the list.
.. In the *Permissions* section, select *Select access*.
.. Select *Select all*.
+
[NOTE]
