...
== Edit RoleBinding Authorized Namespaces

If you use authorized namespaces and want to use persistence gateway, edit the RoleBinding that you create during the configuration of xref:install-helm.adoc#optional-configure-authorized-namespaces[authorized namespaces] by adding a new `rtf-persistence-gateway` subject:
