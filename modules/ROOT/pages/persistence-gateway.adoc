...
Before enabling persistence gateway, ensure that you have created a database to serve as the data source for data stored by Persistence Gateway. This database must be compatible with PostgresSQL 9 and above. The database user that the persistence gateway service uses to connect to the database must have write and update permissions.

During configuration, the persistence gateway service creates the required database schema. Afterwards, when an application deployed to Anypoint Runtime Fabric is configured to use persistent object storage, the persistence gateway service writes the necessary rows to the dataabase.
...
+
If you are using Runtime Fabric on VMs / Bare Metal, this command must be performed on a controller node.
