...
* If your PostgresSQL user account password contains special characters, make sure to encode your connection URL before pushing it as Kubernetes secret.
...
Modify these fields based on your environment:
...
| `spec.objectStore.topologySpreadConstraints.maxSkew` | Describes the degree to which pods may be unevenly distributed. | 1
| `spec.objectStore.topologySpreadConstraints.topologyKey` | Key that understands which criteria to use to distribute pods. | topology.kubernetes.io/zone
| `spec.objectStore.topologySpreadConstraints.whenUnsatisfiable` | Indicates how to deal with a pod if it doesn't satisfy the spread constraint. | ScheduleAnyway
|===
...
Persistence Gateway stores application data using these two tables:
...
If you're using a version of the Runtime Fabric agent higher than 2.6.22, the `id` parameter in the `CREATE TABLE IF NOT EXISTS items` statement no longer applies. For previous versions of the Runtime Fabric agent 2.6.22, the `id` parameter is available in the items table.
