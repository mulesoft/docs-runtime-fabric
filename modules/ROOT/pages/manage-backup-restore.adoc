...
Use these procedures to back up and restore Runtime Fabric services and the required state necessary for all deployed applications and their configurations.
...
For regular Kubernetes clusters, this is backed up:
...
For OpenShift clusters, this is backed up:
...
Runtime Fabric does not restore these control plane changes if you make them after you take a backup:
...
To create a backup, run this command:
...
[IMPORTANT]
--
Before you restore a cluster, review this information:
...
In this example, you have a Runtime Fabric with a set of Mule applications and configurations deployed on cluster (named `cluster-1`) inside an OpenShift cluster. To create a backup and perform a restore, follow these steps:
...
[start=2]
. Shutdown `cluster-1`.
. Delete all Mule application deployments by running this command:
+
[source,copy]
----
...
[[backup-restore-rtfctl]

== Commands for Backing Up and Restoring a Runtime Fabric with RTFCTL
