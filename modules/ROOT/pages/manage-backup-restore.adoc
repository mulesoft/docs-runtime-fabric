...
[NOTE]
The state of Runtime Fabric is distributed across its controller nodes. This procedure backs up and restores
the required state necessary to restore all deployed applications, management services, and their configurations. 
Individual node backups and restores are not supported.
