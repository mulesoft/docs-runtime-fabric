...
Starting `rtfctl` version <tbd>, if you use an rtf-agent version higher than <tbd>, the `rtfctl` backup bundle does not contain the Mule app deployments. The deployments are synched from the Runtime Manager UI at the time of restoration.
