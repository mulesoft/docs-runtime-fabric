...
[NOTE]
Backup operations are only performed at the cluster level. Backups cannot be performed for individual nodes.
...
[NOTE]
Restore operations are only performed at the cluster level. You cannot restore individual nodes.
