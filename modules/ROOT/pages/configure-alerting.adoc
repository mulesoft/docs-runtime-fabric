...
|===
|Alert | Description
| `docker` | Sends an error when docker daemon is down.
...
| `filesystem` | Sends a warning when > 80% used. Triggers a critical error when > 90% used.
| `high-cpu` | Sends a warning when > 75% used. Triggers a critical error when > 90% used.
| `high_memory` | Sends a warning when > 80% used. Triggers a critical error when > 90% used.
| `influxdb_health_batch` | Triggers an error if InfluxDB is down or if there is no connection between Kapacitor and InfluxDB.
| `ingress` | Triggers a critical error when > 80% used.
| `kubernetes_node` | Sends a warning when > 90% used. Triggers a critical error when > 95% used.
| `muleapps` | Triggers a critical error when > 90% used. 
| `network_params` | Triggers a critical error if networking is disabled on a node. 
| `uptime` | Sends a warning when the system is unaccessible for longer than 5 min.
|===
