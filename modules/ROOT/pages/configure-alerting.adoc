...
You can send Runtime Fabric alerts from a designated alert sender email address through your SMTP server. You can configure only one alerts email recipient.
...
[NOTE]
--
If you customize any Runtime Fabric email alerts, appliance upgrades overwrites the customizations. 
--
...
----
. Check that the alert is recreated and the status/executing values are enabled/true:
+
----
kubectl -n monitoring exec <pod name> -c alert-loader -- kapacitor -url http://localhost:9092 list tasks
----
