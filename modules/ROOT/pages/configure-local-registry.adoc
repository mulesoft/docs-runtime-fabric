...
[start=6]
. Following the *Helm* path instructions, download the values.yml file to modify its values. Specifically, for the private registry configuration, update `rtfRegistry` with your local docker server, and `pullSecretName` parameter. 
