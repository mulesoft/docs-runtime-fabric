...
You should see a "LOGIN SUCCEDDED" message
----

. To pull and synchronize images, login into your own private container registry, run the following command, replacing the `<docker-server>` where appropriate:
+
...
# docker push localhost:5000/mulesoft/rtf-agent:v1.12.0
----

. Please repeat above steps for all the other dependency images (resourceFetcher, clusterOps etc.) based on the versions published in Release Note. Finalize the Runtime Fabric installation, and configure it to pull from your local registry:
+
----
