...
. If you use authentication to access your registry, create the required secret on the rtf namespace:
+
----
# kubectl create namespace rtf
# kubectl create secret docker-registry regcred --namespace=rtf --docker-server= --docker-username= --docker-email= --docker-password= #COMPLETE THE COMMAND WITH APROPRIATE CREDENTIALS
