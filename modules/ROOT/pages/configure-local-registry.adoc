...
+
If you set up a local registry, you can upgrade to a version later than 1.12.0 without needing to re-install Runtime Fabric.
+
You cannot add a local registry to an existing Runtime Fabric instance (regardless of its version) even if you upgrade that installation to 1.12.0 or later.
