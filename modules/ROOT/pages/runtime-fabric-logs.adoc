...
*** For example, to configure log forwarding with Splunk:

.... If you are using Mule Runtime version < 3.8, update the Mule Agent version using the ./amc_setup -U command. For more details, please read the following documentation: https://docs.mulesoft.com/runtime-manager/installing-and-configuring-runtime-manager-agent.
.... Configure Splunk Token

..... After installing Splunk on-premises or accessing the cloud version, log in.
..... Navigate to Data Inputs -> HTTP Event Collector.
..... Create a token using the default values and enable it.

.... Configure Mule Runtime

..... Select the Mule on-premises server in the Runtime Manager, select Manage Server, and then Plugins. Splunk and ELK configuration options are displayed.
..... Select Splunk.
..... Copy the Splunk token into Token. Optionally, configure the Splunk Source, Splunk Index, and Source Type. Select a logging Level.

.... Test

..... Create a simple Mule application and publish it.
..... Send some requests and check Splunk. To check the logs, you have to go to Search & Reporting in Splunk GUI:
..... Then click Data Summary to choose the Host/Sources/Sourcetypes.
...
** How do I split RunTime Fabric logs from rest of the logs in Rsyslog Server? To split RunTime Fabric logs from the other Linux system logs, refer to 
