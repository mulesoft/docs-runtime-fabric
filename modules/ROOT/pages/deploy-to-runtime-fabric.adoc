...
Watch this video to see how to deploy a Mule application to Runtime Fabric using Runtime Manager:
...
Before deploying an application to Anypoint Runtime Fabric, perform this:
...
. Click *Choose File* to select the application to deploy, and then select one of these options:
+
...
Use these steps to configure a public endpoint for an application deployed to Runtime Fabric.
...
. Click the *JVM* tab. 
. In the *Java Virtual Machine arguments* field, add one of these arguments:
...
. Run this cURL command for verification:
+
[source,copy]
--
...
--
+
In this example, `{application-path-from-runtime-manager}` is set to `https://newapp.example-rtf.dev`, and `192.168.64.14` is the IP address of a controller machine in your cluster.
+
[source,copy]
--
...
When deploying applications, you may encounter these issues:
