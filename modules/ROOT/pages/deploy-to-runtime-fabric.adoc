...
[NOTE]
To access a Runtime Fabric instance using more than one FQDN, add additional DNS entries in the SubjectAltName (SAN) certificate property. If a certificate has multiple DNS entries specified in the SAN property, the available URLs are displayed in the *Public endpoint* section. Each deployed application can only be addressed using a single FQDN.

. Select *Ingress*.
. Enable the *Enable inbound traffic* checkbox.
. From the *Domain* drop-down list, choose the appropriate domain.
+
. Optionally, enable or disable the following options:
+
