...
. To store data and states across applications, select *Use Persistent Object Storage*.
+
This option is only available on Mule runtime engine, version 4.2.1 or later. If this option is unavailable, contact your Anypoint Runtime Fabric administrator. See xref:persistence-gateway.adoc[Persistence Gateway] for more information.
