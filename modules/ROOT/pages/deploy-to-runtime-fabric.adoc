...
[Note]
New deployments are only possible with Mule runtime engine versions that are within standard support or extended support. You can specify specify the versions through the API or in the Runtime Manager UI. After a version is within the EOL support, redeployments and restars are still possible but no net new deployments are allowed. 
