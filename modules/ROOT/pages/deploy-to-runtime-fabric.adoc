= Deploying Mule Apps to Runtime Fabric Using Runtime Manager
...
[NOTE]
include::cloudhub-2::partial$caveats.adoc[tag=softDelete]
...
Watch the following video to see how to deploy a Mule application to Runtime Fabric using Runtime Manager:

++++
include::_partials/rtf-deploy-mule-apps-rtm.html[]
++++
...
To learn more about Mule runtime versions supported by Runtime Fabric, refer to xref:release-notes::runtime-fabric/runtime-fabric-runtimes-release-notes.adoc[Anypoint Runtime Fabric Patch Update Release Notes].

[Note]
You can deploy Mule apps with Mule runtime engine versions that are within the standard support or extended support life. Specify the versions through the API or in the Runtime Manager UI. With a Mule runtime engine version in EOL (end of life) support, you can redeploy and restart Mule apps but can not deploy new Mule apps. 
...
[WARNING]
====
If there is an insufficient number of worker nodes or resources available on the worker nodes needed to deploy each replica, some replicas in the deployment remain in *Pending* status. The deployment status transitions to *Running* after all replicas are deployed on different worker nodes.
====
...
Anypoint Monitoring provides access to log data for applications deployed to Runtime Fabric. To access logs with Anypoint Monitoring, you need an https://www.mulesoft.com/anypoint-pricing[Anypoint Platform Integration Advanced] subscription.
...
[NOTE]
====
The runtime environment caches application artifacts based on their Maven coordinates (`groupId:artifactId:version`). If you re-publish an Exchange asset under the same version, the cached artifact is served instead of the updated one. Always increment the version number when artifact content changes, or use the `-SNAPSHOT` qualifier for mutable artifacts during development. For details, see xref:deploy-index.adoc#_exchange_asset_caching[Exchange Asset Caching].
====
...
* If the application encounters an error during startup, the deployment stops and the status updates to *Failed*. An error message preview is displayed when clicking on the application and viewing the right sidebar. To view the entire message, click on the *Manage Application* button.
...
* If a re-deployment to an existing running application fails, the previous application remains running and active. This behavior enables zero-downtime application re-deployments. Runtime Manager reports the details and status of the failed deployment to help resolve the failure.
