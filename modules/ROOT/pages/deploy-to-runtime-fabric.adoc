...
You must first xref:enable-inbound-traffic.adoc[enable inbound traffic manually]. Ensure that your application is configured to accept connections on port 8081.
...
+
Or, to enable inbound traffic for an existing application, choose the appropiate domain. 
. To disable inbound traffic for a new application, leave the domain selection empty. 
+
To disable inbound traffic for an existing application, clear the domain selection.
+
