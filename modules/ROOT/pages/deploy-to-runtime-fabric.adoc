...
+
_Result_: The *Deployment Target*, *Ingress*, and *Properties*, and *JVM* tabs are displayed. Use these tabs to enter additional configuration information for your application.
...
** To enable Mule clustering across each replica of the application, enable *Run in Runtime Cluster Mode*.
** To enforce the deployment of replicas across different nodes, select *Enforce deploying replicas across nodes*. This option is only available for Runtime Fabric version 1.5 or later.
+
[WARNING]
This can cause some replicas to be held in *Pending* status if there are not enough worker nodes on which to deploy distinct replicas.
