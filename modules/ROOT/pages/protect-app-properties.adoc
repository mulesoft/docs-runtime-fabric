...
. Depending on how many properties you want to protect at once, take one of these actions:
+
...
In this example, the value for `dbPassword` is protected but those for `dbUsername` and `environment` are not:
