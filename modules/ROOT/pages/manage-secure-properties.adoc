...
. Run this command to set a secure property to the Runtime Fabric under a specific environment, substituting the placeholder fields:
+
[source,copy]
----
...
. Log into a controller node where `rtfctl` is installed. 
. Run this command to view the secure properties applied to the Runtime Fabric under a specific environment, substituting the placeholder fields:
+
[source,copy]
----
...
You can access secure properties from inside your Mule application using `$key`, which is the key provided in the `rtfctl apply` command. You can retrieve the key the same way you retrieve any other xref:mule-runtime::configuring-properties.adoc[configuration property].
...
. Log into a controller node where `rtfctl` is installed. 
. Run this command to remove a secure property from the Runtime Fabric in a specific namespace:
+
[source,copy]
----
./rtfctl delete secure-property <my_key> -n <namespace>
----
+
