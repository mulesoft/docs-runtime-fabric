...
Custom Log4j configurations are enable by default. When you xref:install-helm.adoc[install Runtime Fabric using Helm], the parameter is enabled in `Values.yaml`. Even though the value is set to `true`, you must add an appender. To disable Log4j, set the value to `false`.
