...
. On the controller node acting as the leader during installation (the installer node), run this command: 
+
[source,copy]
----
...
[NOTE]
====
For GKE customers, the ingress controller included with GKE will provision a separate HTTP load balancer per application by default. Please read this link:https://help.salesforce.com/s/articleView?id=001121607&type=1[KB article] for more details.
====
// end::ingressResource[]
