...
For Runtime Fabric usage reports, see xref:general::usage-metrics.adoc#rtf-compliance-tracking[Runtime Fabric Compliance Tracking].

// tag::instructionsMonitor[]
For instructions on how to monitor your usage and get alerts if your usage exceeds your license, refer to https://help.salesforce.com/s/articleView?id=001115241&type=1[Cronjob Script to Monitor RTF Appliance CPU of Mule Apps].
...
When estimating the CPU and memory required for worker nodes, each application replica includes an Anypoint Monitoring sidecar container that streams application metrics to the control plane. The resource allocation for this container varies depending on enabled features such as Anypoint Monitoring logging and tracing. As a baseline, plan for at least 0.05 vCPU cores and 50 Mi of memory per replica, though actual usage can be higher when additional monitoring features are enabled.
// end::vcpuAllocation[]
// tag::cpuBursting[]
...
* Each application replica includes an Anypoint Monitoring sidecar container that streams application metrics to the control plane and cannot be disabled. The CPU and memory reserved for this container vary depending on the monitoring features enabled for the application (such as tracing). As a baseline, plan for at least 50m CPU and 50Mi memory per replica, though actual values can be higher.
...
The minimum values for the amount of memory allocated to each replica of a Mule application or API gateway is:

* 0.7 GiB memory for Mule 4

When Anypoint Monitoring is enabled, memory is reserved for the Anypoint Monitoring sidecar container on each application replica. The actual memory allocation varies depending on enabled monitoring features such as tracing. Plan for a baseline of at least 50Mi per replica, with higher allocations when additional features are active.
