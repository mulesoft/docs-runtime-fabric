...
* CPU performance depends on the profile of the Mule app (such as IO, CPU and memory), as the performance is still subject to the container computing power. Therefore, a throughly performance test is necessary when allocating CPU resource on Runtime Fabric.
