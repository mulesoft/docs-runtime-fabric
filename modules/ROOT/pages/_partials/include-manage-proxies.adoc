...
The rtf-agent pod and any Mule app that needs the proxy applied requires to be restarted. Although no downtime occurs if you are running multiple replicas, you should perform HTTP proxy updates during a maintenance window. 
