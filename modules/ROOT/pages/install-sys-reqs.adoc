...
If using CentOS 7.6.x or later, perform a full installation.
...
Runtime Fabric reserves 0.3 cores per worker node for internal services. This does not account for hyperthreading - Runtime Fabric requires dedicated cores. This is important 
when determining the amount of CPU cores and worker nodes needed.

For fault tolerance, run different sets of controller nodes and worker nodes on separate physical machines.
