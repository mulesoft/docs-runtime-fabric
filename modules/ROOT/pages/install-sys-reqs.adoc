...
The Runtime Manager UI displays the amount of available CPU across worker nodes. For example, you might have a cluster with 3 worker nodes and 2 cores for each worker node. If Runtime Fabric reserves a total of 0.9
cores (0.3 x 3), the number of available cores displayed in the Runtime Manager would be 5.1 instead
of the full 6 cores.
