...
Anypoint Runtime Fabric support development and production configurations. The requirements for each configuration are described below. See xref:architecture.adoc[Runtime Fabric Architecture] for a description of these environments.
...
The minimum number of controller VMs in a production environment is three. The maximum is five. These requirements reflect the minimum requirements to maintain high-availability and ensure system performance.
...
Runtime Fabric requires at least one controller VM in a development environment. The maximum supported is 5.
...
To keep your Runtime Fabric installation running during upgrades requires at least one additional worker VM, including hard disks. This VM should only be used when:
