...
Run rtfctl report tasks in parallel by using the `workers` flag that enables how many tasks to run in parallel. The default value is 10.
...
Select which application namespaces you want to include in your report besides from the global rtf installation namespace by using the `<rtf_namespace>`. Using this flag enables you to choose what to include in your reports, and returns a faster report. 
