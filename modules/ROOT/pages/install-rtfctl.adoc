...
----
Note that if Runtime Fabric is installed via Helm, you must update the `mule-license` property in the Helm values and not not use `rtfctl apply mule-license`.
