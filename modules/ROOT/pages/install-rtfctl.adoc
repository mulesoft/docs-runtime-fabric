...
The `rtfctl` command line tool (from vX.X.X and above) audits the rtfctl operations on the cluster. Helping the cluster admins/users to track operations that impact RTF core software.
Some key points:
1. Audit logs are stored in a kubernetes secret in `rtfctl-audit` namespace.
2. Audit logs are cluster scoped and not attached to RTF core software lifecycle.
3. Execution of commands will continue to function if fails to audit the operation.
...
Note:
- Currently Audit logs rollover/rotation is not supported.
