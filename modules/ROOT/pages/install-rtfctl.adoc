...
ON VMs / Bare Metal  the `rtfctl` tool must be run from a controller node. It will self detect the required configuration to manage the cluster. 

On Self-Managed Kubernetes `rtfctl' consumes kubeconfig files as described in https://kubernetes.io/docs/concepts/configuration/organize-cluster-access-kubeconfig/[Organizing Cluster Access Using kubeconfig Files]. Defer to this method to configure which cluster you want to manage.
