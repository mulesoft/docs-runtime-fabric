...
* *Runtime Fabric on Self-Managed Kubernetes*: a version of Runtime Fabric that you install on an existing Kubernetes environment that you operate and manage. This version supports Amazon Elastic Kubernetes Service (EKS), Azure Kubernetes Service (AKS), and Google Kubernetes Engine (GKE).
...
| Self-Managed Kubernetes | Requires an IT administrator to create and configure a Kubernetes cluster on AKS, EKS or GKE. This persona is also responsible for installing Runtime Fabric and managing Kubernetes. | Creates Runtime Fabrics, deploys projects, and performs other tasks within Anypoint Platform
...
You must provide your instances of Kubernetes and Docker via EKS, AKS or GKE clusters. | Included
