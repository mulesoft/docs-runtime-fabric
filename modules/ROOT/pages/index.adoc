...
:page-notice-banner-message: For documentation about Runtime Fabric on VMs/Bare Metal, see version 1.13 or earlier.  
...
If you work in public clouds and have some level of expertise in managing a Kubernetes cluster, run xref:1.13@runtime-fabric::index-self-managed.adoc[Runtime Fabric on public cloud K8s clusters]. Runtime Fabric supports:
...
If you prefer to standardize your own bare-metal solutions using public K8s distributions, run https://www.youtube.com/watch?v=MYOeX5qjYew[Runtime Fabric on supported IBM Openshift offerings^]. Depending on your affinity to public clouds and in-house Kubernetes expertise, the following self-managed Red Hat OpenShift offerings can enable both On-Prem and public cloud bare-metal solutions:
