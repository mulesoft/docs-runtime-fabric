...
Each of these options has its own system requirements and installation procedures. See xref:index-self-managed.adoc[Runtime Fabric on Self-managed Kubernetes] and xref:index-appliance.adoc[Anypoint Runtime Fabric on VMs / Bare Metal] for more information on each option.
