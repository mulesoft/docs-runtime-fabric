...
+
Run no more than 20 - 25 apps per core, up to a maximum of 40 apps per node, to allow core sharing across apps when needed for bursting. This ensures the Runtime Fabric’s internal components that run on each worker node are not overloaded by too many replicas.
