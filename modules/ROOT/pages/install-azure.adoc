...
This topic describes how to install Anypoint Runtime Fabric on VMs / Bare Metal on your Azure account. Note that you can also use Elastic Kubernetes Service (EKS), Azure Kubernetes Service (AKS), or Google Kubernetes Engine (GKE) to install xref:index-self-managed.adoc[Runtime Fabric on Self-Managed Kubernetes].
