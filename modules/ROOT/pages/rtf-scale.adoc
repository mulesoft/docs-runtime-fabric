...
Runtime Fabric functionality and management of application deployments is not affected by this:
...
Review these benchmarked metrics to understand various management metrics when compared between a nearly empty cluster and a cluster running 8000 deployments. The cluster configuration used is Runtime Fabric on AWS EKS cluster:
...
Review these scale considerations:

* Runtime Fabric agent requires more than default memory and CPU to manage clusters with large-scale deployments, you can modify such values using the information in this table. For a cluster with 8000 deployments, use at least:
+
[%header%autowidth.spread]
|===
...
|===
+
* The default resource allocation supports up to 2K apps / 75 nodes.
