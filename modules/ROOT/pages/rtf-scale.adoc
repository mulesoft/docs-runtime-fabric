...
| Horizontal Pod Autoscaling (HPA) | With Runtime Fabric on EKS occasional traffic failure of approximately 0.09% may occur when scaling replicas from 1 to 8. These errors are specific to the pod scaling window, so you may experience traffic failure during scaling operations. | Recommended
|===
