...
[NOTE]
====
Starting with Runtime Fabric agent version 3.0.0, the `am-log-forwarder` role is no longer used as log forwarding occurs exclusively through application sidecar containers. For more information, refer to xref:migrate-log-forwarding-3-0-0.adoc[Migrate Log Forwarding Before Upgrading to Runtime Fabric Agent Version 3.0.0].
====
