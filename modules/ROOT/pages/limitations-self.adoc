...
[NOTE]
====
Starting with Runtime Fabric agent version 3.0.0, the `am-log-forwarder` role is no longer used as log forwarding occurs exclusively through application sidecar containers. For more information, refer to xref:use-anypoint-monitoring.adoc#log-forwarding-in-runtime-fabric-3-0-0[Log Forwarding in Runtime Fabric 3.0.0].
====
