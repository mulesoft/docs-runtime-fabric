...
[NOTE]
====
Starting with Runtime Fabric agent version 3.0.0, the `am-log-forwarder` role is no longer used as log forwarding occurs exclusively through application sidecar containers. For more information, refer to xref:migrate-log-forwarding.adoc[Migrate Log Forwarding Before Upgrading Runtime Fabric].
====
