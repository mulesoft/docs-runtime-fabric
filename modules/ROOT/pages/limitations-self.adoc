...
| Replicas per application | Runtime Fabric supports a maximum of 16 replicas per application in all forms except when you use the usage pricing and Horizontal Pod Autoscaling feature. In this case, the maximum number of replicas per application is 8.
...
Anypoint Runtime Fabric works on any Linux distribution supported by:

* Alibaba Cloud Container Service (ACK)
* Amazon Elastics Kubernetes Service (EKS)
...
Runtime Fabric agent supports most Linux distributions. However, you must conduct your own testing to ensure that Runtime Fabric is compatible with your chosen operating system. Also validate xref:mule-runtime::hardware-and-software-requirements.adoc[Mule runtime engine compatibility] and xref:monitoring::am-installing.adoc#os-support[Anypoint Monitoring compatibility] with your operating system.
...
[NOTE]
====
Starting with Runtime Fabric agent version 3.0.0, the `am-log-forwarder` role is no longer used as log forwarding occurs exclusively through application sidecar containers. For more information, refer to xref:upgrade-pre-requisites.adoc#migrate-log-forwarding-3-0-0[Migrate Log Forwarding Before Upgrading to Runtime Fabric Agent Version 3.0.0].
====
...
[IMPORTANT]
====
The default behavior of the ingress controller included with GKE is to provide a separate HTTP load balancer for each application. If you find this behavior undesirable, explore possible solutions or consider using another ingress controller.
====
