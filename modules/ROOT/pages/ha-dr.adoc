...
Cloud providers such as AWS and Azure,have data centers grouped by geographical Regions. Multiple data center within each region are classified asAvailability Zones. Redundancy can be achieved by creating a private network and using virtual machines across multiple availability zones. More details on the concepts of regions and availability zones  can be found described by AWS and Azure. 

Note, the number of availability zones is dependent on the region selected for deployment. For high availability, it’s recommended to select a region that has at least 3 availability zones.

For detailed Runtime Fabric architecture specifications for development and production environments, refer to https://docs.mulesoft.com/runtime-fabric/1.3/architecture.
High Availability
Availability across nodes
By default, Runtime Fabric is built to automatically detect and redeploy applications to available resources in the event of a node failure. To take advantage of this measure, Runtime Fabric should have at least one worker node’s worth of unused capacity. With the production configuration of Runtime Fabric, the controller nodes are also able to tolerate one node failure without impacting availability. 

Availability Zones
Availability zones (AZs) are data centers within a geographical region. . Regions are different geographic locations each hosting one or more availability zones. High-speed, low latency private networks can span multiple availability zones within a region.
...
To achieve high availability when using AWS or Azure, you must deploy Runtime Fabric on a VPC or VirtualNetwork with a minimum of 3 subnets, one for each required availability zone. You can distribute the deployment of worker nodes and controller nodes to each availability zone. This will automatically protect the Runtime Fabric from a failure within an availability zone.. 
...
Node Configuration 
For fault tolerance, a minimum of 3 controller nodes is required when spinning up a Runtime Fabric cluster. For the basic high availability pattern, a minimum of 3 worker nodes is required. The DR environment must duplicate the production environment.

For production environments, you must set up and install a minimum of 6 worker nodes and 6 controller nodes into 3 different Availability Zones. The DR environment setup must be an exact replica of the production environment.
...
At the node level, controllers are replicated to at least 3 nodes for a minimal production setup. To remain healthy, controllers must be in sync within a very small window of time. With 3 controller nodes, you can operate well in the event of 1 controller failure. 

In a production setting, assume a single node might always be unavailable due to an unexpected failure as well as a need to apply patches. Patches require taking a node down and rebooting, necessitating a 1 node buffer in order to safely take a node out of rotation, apply upgrades, bring it back online, and do a rolling upgrade at the node level.
...
Within RTF itself, apart from the configuration, there is nothing that needs to be managed and replicated between nodes because each deployment is self-contained. When you deploy an app to  RTF using RTM, through any method RTF provides, including Maven, UI, and APIs, everything needed for the app to run is contained in the deployment manifest sent to RTF. With a programmatic deployment, you can replicate a deployment across 2 different clusters by simply changing the cluster ID and triggering one app deployment that invokes two API calls. This deploys 2 different instances of the app with the same configuration. The only difference is where each app is deployed. 
...
Disaster Recovery 
Runtime Fabric requires separate clusters to be deployed in an active-active configuration for production and DR environments. 

Note:
For an active-active configuration, the DR node mirrors the production node, which doubles the overall required capacity.
