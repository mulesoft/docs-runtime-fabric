...
For Runtime Fabric specifications for development and production environments, refer to xref:architecture[Runtime Fabric Architecture].
...
When using a cloud provider like AWS or Azure, high-speed, low latency private networks can span multiple availability zones within a region.

When combined with the availability across nodes, Runtime Fabric can be configured to be highly available within a geographical region by distributing nodes across different availability zones.

Runtime Fabric can be configured according to the type of environment:

* Production environments require a minimum of three availability zones for improved availability.
* DR environments require require a minimum of three availability zones. DR availability zones are in addition to production environment availability zones. Having a DR environment necessitates double the capacity of only having a production environment.
* Non-production environments require a minimum of two availability zones for improved availability.
...
[NOTE]
An environment with two availability zones does not provide high availability, which requires three availability zones. In the case of two availability zones, you can create a DR environment in a third availability zone for high availability.

Deploy applications and services with two replicas. Each replica should be scheduled to run in a separate node. Note that with Runtime Fabric, the scheduling of replicas across nodes is performed on a best effort basis, and can vary depending on worker node availability and capacity.
...
== Disaster Recovery 
Runtime Fabric allows separate clusters to be deployed in an active-active configuration or active-passive configuration for production and DR environments. In the active-active case, the DR node mirrors the production node, which doubles the overall required capacity.
...
=== Licensing Core Count with Active-Passive Standby

If you are maintaining a Hot-Standby for your DR site in an active-passive model, make sure that you do not completely consume your vCore count on servers you do not plan to use except when patching or in other unplanned emergency. You must maintain compliance with your MuleSoft licensing contract.

To ensure licensing compliance, you can create an identical stand-by secondary RTF cluster in your DR site with no Mules deployed on it, and fail-over in the event of a disaster via one of the following methods:

* Restore from an earlier backup taken on the active RTF. For information on how to back up and restore Runtime Fabric, refer to xref:https://docs.mulesoft.com/runtime-fabric/1.3/manage-backup-restore[Backup and Restore Runtime Fabric].
+
** You must have regular backups executed and transferred to the DR site. 
** The secondary RTF assumes the role of the primary RTF, so the primary RTF must be disconnected from Anypoint control plane prior to applying the backup on the secondary RTF.
** Both RTFs would need to have the same configuration (same size nodes, same size disks, same # of cores, etc).
* Use the https://anypoint.mulesoft.com/exchange/portals/anypoint-platform/f1e97bc6-315a-4490-82a7-23abe036327a.anypoint-platform/amc-application-manager/version/2.0/pages/home/[Application Manager API] to automate the setting of each application's deployment target from the primary RTF to the secondary RTF.
+
** You are updating the desired state in the Anypoint control plane to move applications from one target (the primary RTF) to the secondary target (the passive RTF).
** The configuration of the environments, business groups, and internal load balancer should be identical between both RTFs.

In either case:

* You are responsible for cutting over traffic from the primary RTF to the secondary RTF.
* The secondary RTF can be spun up without impact to Mule licensing as long as there are no Mule applications running on it.
* During fail-over, the primary RTF must be disconnected from serving traffic in order to stay compliant -- under normal licensing terms. 
