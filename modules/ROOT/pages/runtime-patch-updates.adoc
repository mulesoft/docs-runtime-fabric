...
Starting with Mule runtime engine 4.5.0, autoupgrades are disabled automatically. If a Mule app is successfully deployed through the API without a build tag, the latest build is auto selected. The grace period section is only valid for versions earlier than 4.5.0. With versions later than 4.5.0, all builds under a Mule runtime engine version are available for deployment.
