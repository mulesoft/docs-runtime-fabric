...
The following table describes the autoupgrade behavior of Mule runtime patch updates:

[%header%autowidth.spread]
|===
|Mule runtime patch updates | Releases before October 3, 2023 | Releases after October 3, 2023
| Versions 4.4 and earlier |The deployment defaults to or autoupgrades to the latest patch of the selected minor. | Anypoint Platform API requires passing a valid and supported build tag in the deployment specification. If you pass a wrong build tag, the deployment fails.


If you deploy a Mule app without the build tag, the deployment defaults to or autoupgrades to the latest patch of the selected minor.
| Versions 4.5 and following | - | Mule runtime image names have the build tag built within them, so you do not have to pass a separate build tag. You are not defaulted to or autoupgraded to the latest patch of the selected minor.
