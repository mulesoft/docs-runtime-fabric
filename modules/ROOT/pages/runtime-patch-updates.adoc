...
Starting Mule runtime engine 4.5.0, autoupgrades are disabled automatically. Any Mule app deployment through the API without specifying the build tag goes through successfully and the latest build is auto selected. The grace period section is only valid for versions < 4.5.0. With the version 4.5.0+, all builds under a Mule runtime engine version is available for deployment.
