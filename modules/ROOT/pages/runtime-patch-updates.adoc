...
Version rewrite used to override the build tag specified in a deployment request if the build tag was out of support. Starting with Mule runtime engine version 4.5, version rewrite does not override any specified tags for any version. However, if a Mule app is deployed through the API without a build tag or with an incorrect build tag, the latest build is auto-selected.
