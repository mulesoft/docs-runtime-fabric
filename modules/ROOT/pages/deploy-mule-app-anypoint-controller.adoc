...
* Mule runtime versions 4.6 and higher
...
[IMPORTANT]
Backup and restore functionality (during Limited GA phase) is not supported. Plan your disaster recovery strategy accordingly, leveraging your Git repository as the source of truth for application configurations.
...
* `volumes` - Defines the storage volumes available to the application, such as Kubernetes Secrets, ConfigMaps, persistent volumes, or external secret stores.
* `volumeMounts` - Specifies where to mount the volumes inside the application container and makes the data accessible at the specified file path.
