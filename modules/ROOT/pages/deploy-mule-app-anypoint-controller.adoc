...
[IMPORTANT]
Backup and restore functionality (during the Limited GA phase) is not supported. Plan your recovery strategy accordingly, leveraging your version control repository as the source of truth for application configurations.
...
For more information, see xref:install-multiple-instances.adoc[Install Multiple Instances of Runtime Fabric].
...
. Update your `values.yaml` file with the appropriate configuration:
...
. Create the namespace and label it with the Runtime Fabric namespace:
+
[source,bash]
----
...
+
[IMPORTANT]
The `rtf.mulesoft.com/agentNamespace` label tells the Runtime Fabric agent which MuleApplication Custom Resources to manage. Only one Runtime Fabric instance can manage applications in a given namespace.
...
When you apply the Mule application Custom Resource, Anypoint Controller perform these actions:

. Detects the new MuleApplication resource.
. Validates the configuration.
. Adds a label to the namespace for synchronization tracking (`rtf.mulesoft.com/role: workers`).
...
The controller watches namespaces that are labeled with `rtf.mulesoft.com/agentNamespace: <rtf-namespace>`, where `<rtf-namespace>` is the namespace where the Runtime Fabric core software is installed. The agent running in that namespace manages Mule application Custom Resources in the labeled application namespace.
...
[IMPORTANT]
Deletion is permanent. Ensure your version control repository contains the application configuration if you need to redeploy later.
...
. Store MuleApplication manifests in a version control repository.
. Define branch strategies (for example, `dev`, `staging`, `production`).
. Use pull requests or merge requests for configuration changes.
...
. Store your manifests in version control:
+
[source,console]
----
my-mule-apps/
...
To enable the Runtime Fabric agent to manage MuleApplication Custom Resources in a namespace, you must label the namespace with `rtf.mulesoft.com/agentNamespace=<rtf-namespace>`. Only one Runtime Fabric instance can manage applications in a given namespace.

* Namespace Flexibility +
You can create MuleApplication Custom Resources in any Kubernetes namespace, as long as the namespace is labeled with `rtf.mulesoft.com/agentNamespace: <rtf-namespace>`. A controller running in a given Runtime Fabric agent pod only watches for MuleApplication Custom Resources in namespaces that have this matching label.
...
For more information about configuring ingress for Runtime Fabric, see xref:configure-ingress.adoc[Configure Ingress].
...
By default, the log level is set to `info`, which does not emit detailed information for diagnosis. To enable debug logs for more detailed troubleshooting, add the following environment variables to the `app-config` ConfigMap and restart the agent deployment:
...
. Check <<check-controller-logs>> for errors.
====
