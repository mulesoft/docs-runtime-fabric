...
. <<define-muleapplication-custom-resource,Define Mule Application CR>>
...
[source,bash]
----
helm install runtime-fabric <chart-repo>/rtf-agent \
  -f values.yaml \
  -n <rtf-namespace>
----
====
...
[source,bash]
----
helm upgrade runtime-fabric <chart-repo>/rtf-agent \
  -f values.yaml \
  -n <rtf-namespace>
----
====
...
* <<delete-a-mule-application,Delete a Mule Application>>
...
| KubernetesTemplate | Not supported | Define all Kubernetes-specific configurations directly in the Mule application CR fields (`spec.pod`, `spec.service`, `spec.deployment`)
