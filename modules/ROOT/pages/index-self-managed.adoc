...
Linux-based operating systems are required for all nodes that run Anypoint Runtime Fabric components on self-managed Kubernetes clusters.
...
== Runtime Fabric on Self-Managed Kubernetes as a Shared Responsibility
...
* xref:custom-ingress-configuration.adoc[Customizing ingress resources]
* xref:configure-local-registry.adoc[Configuring a local registry to use with Runtime Fabric]
* Log forwarding
* Monitoring
* Network ports, NAT gateways, and proxies
* Container runtime and networking
* Provisioning and management of the Kubernetes environment, including assistance from the following teams in your organization:
...
Adjust the number of nodes and amount of resources allocated according to the amount and type of workload you run on each Anypoint Runtime Fabric for Self-Managed Kubernetes instance.
