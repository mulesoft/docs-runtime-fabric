...
Runtime Fabric on Self-Managed Kubernetes supports any ingress controller that is compatible with the Kubernetes environment on your cloud infrastructure vendor and supports a deployment model where a separate Ingress resource is created per application deployment. In general, most off the shelf ingress controllers support this model.
