...
. Update your `values.yaml` file with any additional required parameters.
. Set `authorizedNamespaces` to `true`, if you’re using authorized namespaces.
. Set `crds.install` to `false`.
. Click *Create*.
...
[NOTE]
====
Starting with Runtime Fabric agent version 3.0.0, the `am-log-forwarder` service account is no longer required as log forwarding occurs exclusively through application sidecar containers. You can omit this service account for new installations of version 3.0.0 or higher.
====
+
