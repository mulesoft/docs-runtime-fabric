...
* Availability zones +
Runtime Fabric spreads Mule application replicas across availability zones. When you enable CPU-based Horizontal Pod Autoscaling (HPA) for a Mule application, Runtime Fabric spreads the autoscaled application replicas evenly across zones. HPA applies to Mule applications, not to the Runtime Fabric core software. See xref:configure-horizontal-autoscaling.adoc[].
