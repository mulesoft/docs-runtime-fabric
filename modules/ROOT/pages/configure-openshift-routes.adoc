...
This documentation also assumes you’re familiar with https://docs.openshift.com/container-platform/4.10/networking/routes/route-configuration.html#nw-ingress-creating-a-route-via-an-ingress_route-configuration[Creating a route through an Ingress object^].
...
<3> TLS is optional. If specified in the template it will create a route with edge termination by default unless you set the `route.openshift.io/termination:` annotation.
...
After you deploy the application, a route will be created to serve traffic to the application as per the template specified.
...
. Follow the steps to create an ingress resource template. 
